...
const yaml = requireFromHere('js-yaml');
...
function parseShippedSkillFrontmatter(skillName) {
    const content = readFileSync(join(SKILLS_DIR, skillName, 'SKILL.md'), 'utf-8');
    const match = content.match(/^---\r?\n([\s\S]*?)\r?\n---(?:\r?\n|$)/);
    if (!match)
        throw new Error(`${skillName} is missing YAML frontmatter`);
...
describe('shipped skill frontmatter', () => {
    ...
    it('keeps model-invoked descriptions within the compact plugin shim budget', () => {
        for (const skillName of shippedSkillNames()) {
            ...
            expect(description.length, `${skillName} description length`).toBeLessThanOrEqual(SHIM_DESCRIPTION_BUDGET);
        }
    });
    it('keeps diagram positive and skip criteria intact within the shim budget', () => {
        ...
        expect(description.length).toBeLessThanOrEqual(SHIM_DESCRIPTION_BUDGET);
