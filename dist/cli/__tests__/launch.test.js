...
import { resolveLaunchPolicy, buildTmuxShellCommand, buildTmuxShellCommandWithEnv, isNativeWindowsShell, wrapWithLoginShell, quoteShellArg, isTmuxAvailable, tmuxExec, } from '../tmux-utils.js';
function mockValidTmuxPane() {
...
describe('runClaude — exit code propagation', () => {
    ...
    describe('inside-tmux policy', () => {
        ...
        it('respawns the current pane on win32 so claude.cmd replaces the launcher', () => {
            ...
            expect(vi.mocked(tmuxExec)).toHaveBeenCalledWith(['respawn-pane', '-k', '-t', '%0', '-c', '/tmp', '--', expect.stringContaining('claude')], { stdio: 'inherit' });
            Object.defineProperty(process, 'platform', { value: originalPlatform, configurable: true });
        });
        it('fails safely without an invoking pane id', () => {
            ...
            runClaude('/tmp', [], 'sid');
            expect(processExitSpy).toHaveBeenCalledWith(1);
            expect(stderrSpy).toHaveBeenCalledWith('[omc] Error: unable to identify the invoking tmux pane; refusing to respawn Claude.');
        ...
        it('authenticates TMUX_PANE against the invoking client instead of targeting it', () => {
            ...
            runClaude('/tmp', [], 'sid');
            const paneQuery = vi.mocked(tmuxExec).mock.calls.find(([args]) => args[0] === 'display-message' && args.includes('#{pane_id}'))?.[0] ?? [];
        ...
        it('forwards effective provider credentials without putting the value on a command line', () => {
            ...
            try {
                runClaude('/tmp', [], 'sid');
                const command = String(vi.mocked(tmuxExec).mock.calls.find(([args]) => args[0] === 'respawn-pane')?.[0].at(-1));
        ...
        it('keeps provider credentials off the native psmux respawn command line', () => {
            ...
            try {
                ...
                expect(envFile).toBeDefined();
                expect(readFileSync(envFile, 'utf8')).toContain(`set "ANTHROPIC_API_KEY=${secret}"`);
                rmSync(dirname(envFile), { recursive: true, force: true });
            }
            finally {
                Object.defineProperty(process, 'platform', { value: originalPlatform, configurable: true });
        ...
        it('keeps non-sensitive forwarded variables in the inline export prefix', () => {
            ...
            try {
                ...
                if (savedNotify === undefined)
                    ...
                    process.env.OMC_NOTIFY = savedNotify;
            }
        });
        it('exits with code 1 on ENOENT', () => {
            const err = Object.assign(new Error('Not found'), { code: 'ENOENT' });
...
describe('buildEnvExportPrefix — quoting', () => {
    ...
    it('preserves nested apostrophes and shell metacharacters literally', () => {
        ...
        expect(buildEnvExportPrefix(['TEST_QUOTE_VAR'])).toBe(`export TEST_QUOTE_VAR='a'"'"'b "$HOME"; $(touch /tmp/should-not-run)'; `);
...
describe('runClaude outside-tmux — env forwarding', () => {
    ...
    it('forwards POSIX credentials through a path-only new-session transport', () => {
        ...
        try {
            ...
            expect(envFile).toBeDefined();
            expect(lstatSync(envFile).mode & 0o777).toBe(0o600);
            expect(readFileSync(envFile, 'utf8')).toContain("export ANTHROPIC_API_KEY='");
            expect(readFileSync(envFile, 'utf8')).toContain('new-session secret');
            rmSync(dirname(envFile), { recursive: true, force: true });
        }
        finally {
            if (savedApiKey === undefined)
    ...
    it('canonicalizes a Windows Path entry to PATH when forwarding the live environment', () => {
        ...
        try {
            ...
            if (savedPathCasing === undefined)
                delete process.env.Path;
            else
                process.env.Path = savedPathCasing;
        }
    });
    it('passes a cmd-friendly raw command string into login-shell wrapping on native Windows', () => {
        ...
        expect(vi.mocked(buildTmuxShellCommandWithEnv)).toHaveBeenCalledWith('claude', ['--print-system-prompt', 'hello world'], expect.objectContaining({ CLAUDE_CONFIG_DIR: 'C:\\Users\\bellman\\config dir' }));
    ...
    it('keeps credential values off the native psmux command line while preserving percent escaping', () => {
        ...
        try {
            ...
            const envFile = envFileMatch?.[1] ?? envFileMatch?.[2];
            expect(envFile).toBeDefined();
            expect(readFileSync(envFile, 'utf8')).toContain(`set "ANTHROPIC_API_KEY=literal%%PATH%%"`);
            rmSync(dirname(envFile), { recursive: true, force: true });
            const envArgs = vi.mocked(buildTmuxShellCommandWithEnv).mock.calls.at(-1)?.[2];
    ...
    it('rejects newline values before composing a native psmux command', () => {
        ...
        vi.mocked(isNativeWindowsShell).mockReturnValue(true);
        try {
            expect(() => runClaude('/tmp', [], 'sid')).toThrow('Native Windows tmux command values cannot contain NUL, CR, or LF characters');
        }
        finally {
            Object.defineProperty(process, 'platform', { value: originalPlatform, configurable: true });
            if (savedApiKey === undefined)
