...
function readInputFile(path) {
    if (!path)
        throw new Error('workflow_input_file_required');
...
function inputObject(value, keys, error = 'workflow_invalid_runtime_config') {
...
function runtimeText(value, limit = 4000) {
...
function privatePath(value, cwd) {
...
function readRuntime(path, cwd) {
    try {
        ...
        for (const [ref, value] of Object.entries(inputObject(raw.profiles))) {
            ...
            if (providerRoute !== 'claude' && providerRoute !== 'glm' && providerRoute !== 'codex')
                throw new Error('workflow_invalid_runtime_config');
            ...
            if (!Array.isArray(profile.files) || profile.files.length > 30)
                throw new Error('workflow_invalid_runtime_config');
            const files = profile.files.map(runtimePath);
            let redactionValues;
            if (profile.redactionValues !== undefined) {
                if (!Array.isArray(profile.redactionValues) || profile.redactionValues.length > 100)
                    throw new Error('workflow_invalid_runtime_config');
        ...
        return { ...(reviewAuthorship ? { reviewAuthorship } : {}), resolveBinding(binding) {
                const authProfile = profiles.get(binding.authProfileRef);
                const capabilityEvidencePath = evidence.get(binding.id);
                if (!authProfile || !capabilityEvidencePath)
        ...
        if (error instanceof Error && error.message === 'workflow_private_runtime_outside_project_required')
            throw error;
...
export async function workflowCommand(args, cwd = process.cwd()) {
    ...
    if (!Object.hasOwn(allowed, operation))
        throw new Error('workflow_unknown_operation');
    ...
    if (operation === 'publish-result') {
        ...
        if (!source || !destination || !isAbsolute(destination))
            throw new Error('workflow_invalid_publication_arguments');
    ...
    const runtime = () => {
        ...
        if (file === undefined)
            return undefined;
        if (readWorkflow(cwd, name).schemaVersion !== 2)
    ...
    if (operation === 'init') {
        ...
        if (profile !== 'claude-glm-codex' && profile !== 'role-substitution')
            throw new Error('workflow_invalid_profile');
        if (profile === 'claude-glm-codex' && flags.has('--bindings'))
            throw new Error('workflow_role_substitution_profile_required');
        ...
        if (profile === 'role-substitution') {
            if (mode !== undefined && mode !== 'balanced')
        ...
        if (profile === 'role-substitution') {
            ...
            for (const role of ['lead', 'implementer', 'reviewer'])
                inputObject(roles[role], undefined, 'workflow_invalid_bindings');
            ...
            name = (await initWorkflow(cwd, plan, options)).plan.name;
    }
    else if (operation === 'run') {
        const selected = runtime();
        if (selected)
            await runWorkflow(cwd, name, selected);
        else
        ...
        if (!reason?.trim())
            throw new Error('workflow_reason_required');
        if (!head || !/^[a-f0-9]{40}(?:[a-f0-9]{24})?$/.test(head))
            throw new Error('workflow_invalid_sha');
        const selected = runtime();
        if (selected)
            await resumeWorkflowTask(cwd, name, positional[1], head, reason, selected);
        else
        ...
        if (role !== 'lead' && role !== 'implementer' && role !== 'reviewer')
            throw new Error('workflow_invalid_substitution_intent');
        ...
        await acceptWorkflowTask(cwd, name, positional[1]);
        ...
        await verifyWorkflow(cwd, name);
    else if (operation === 'review') {
        const selected = runtime();
        if (selected)
            await reviewWorkflow(cwd, name, selected);
        else
            await reviewWorkflow(cwd, name);
    }
        ...
        await adjudicateWorkflow(cwd, name, input);
    }
        ...
        await finishWorkflow(cwd, name);
    else if (operation === 'cleanup')
        cleanup = await cleanupWorkflow(cwd, name);
...
//# sourceMappingURL=team-workflow.js.map