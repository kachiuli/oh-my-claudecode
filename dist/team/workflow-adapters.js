...
function object(value) {
    if (!value || typeof value !== 'object' || Array.isArray(value))
        throw new Error('workflow_invalid_capability_evidence');
    return value;
}
function fileDigest(path) {
    if (!isAbsolute(path) || lstatSync(path).isSymbolicLink() || !statSync(path).isFile())
        throw new Error('workflow_invalid_runtime_file');
    return digest(readFileSync(path));
}
function receipt(path, expected) {
    if (!isAbsolute(path) || lstatSync(path).isSymbolicLink() || !statSync(path).isFile() || statSync(path).size > 64 * 1024)
        throw new Error('workflow_capability_evidence_changed');
    const bytes = readFileSync(path);
    if (bytes.length > 64 * 1024 || digest(bytes) !== expected)
        throw new Error('workflow_capability_evidence_changed');
    try {
...
function captureAuthProfile(profile) {
    const privateValues = [...(profile.redactionValues ?? [])];
    const secrets = (value, sensitive = false) => {
        if (typeof value === 'string' && sensitive && value.length >= 4)
            privateValues.push(value);
        else if (Array.isArray(value))
            for (const entry of value)
                secrets(entry, sensitive);
        else if (value && typeof value === 'object')
            for (const [key, entry] of Object.entries(value))
                secrets(entry, sensitive || /key|token|secret|password|credential|authorization/i.test(key));
    };
    if (profile.files.length > 30)
        throw new Error('workflow_invalid_auth_profile');
    const files = profile.files.map(path => {
        if (!isAbsolute(path) || lstatSync(path).isSymbolicLink() || !statSync(path).isFile() || statSync(path).size > 1024 * 1024)
            throw new Error('workflow_invalid_runtime_file');
        const bytes = readFileSync(path);
        ...
        try {
            ...
            privateValues.push(bytes.toString('utf8').trim());
        }
        ...
        if (parsed && typeof parsed === 'object' && 'env' in parsed) {
            ...
            for (const [key, value] of Object.entries(object(parsed.env))) {
                if (typeof value !== 'string')
                    throw new Error('workflow_invalid_profile_environment');
    ...
    const fingerprint = digest(JSON.stringify({ ref: profile.ref, providerRoute: profile.providerRoute,
        environment: Object.entries(profile.environment).filter((entry) => entry[1] !== undefined)
...
export function fingerprintWorkflowAuthProfile(profile) {
...
function childEnvironment(profile) {
    ...
    for (const [key, value] of Object.entries(process.env)) {
        if (/^(?:PATH|PATHEXT|SYSTEMROOT|WINDIR|TEMP|TMP|HOME|USERPROFILE|APPDATA|LOCALAPPDATA|PROGRAMDATA|PROGRAMFILES|PROGRAMFILES\(X86\)|LANG|LC_[A-Z_]+|TERM)$/i.test(key))
            environment[key] = value;
    }
    const keys = new Set();
    for (const [key, value] of Object.entries(profile.environment)) {
        ...
        if (keys.has(identity))
            throw new Error('workflow_duplicate_profile_environment');
        keys.add(identity);
        if (/^(?:CLAUDECODE|CLAUDE_CODE_ENTRYPOINT|CLAUDE_CODE_SESSION_ID|OMC_TEAM_|NODE_OPTIONS|NODE_PATH)/i.test(key))
            throw new Error('workflow_unsafe_profile_environment');
        const foreign = profile.providerRoute === 'codex' ? /^(?:ANTHROPIC_|CLAUDE_|OMC_GLM_)/i
            : /^(?:OPENAI_|CODEX_)/i;
        if (foreign.test(key) || profile.providerRoute === 'claude' && /^OMC_GLM_/i.test(key))
            throw new Error('workflow_auth_route_mismatch');
        if (profile.providerRoute === 'claude' && /^ANTHROPIC_BASE_URL$/i.test(key) && value
            && value.replace(/\/$/, '') !== 'https://api.anthropic.com')
            throw new Error('workflow_auth_route_mismatch');
        if (profile.providerRoute === 'claude' && /^(?:CLAUDE_CODE_USE_(?:BEDROCK|VERTEX|FOUNDRY)|OMC_EXTERNAL_MODELS_)/i.test(key) && value && !['0', 'false'].includes(value))
            throw new Error('workflow_auth_route_mismatch');
        if (value !== undefined) {
            if (process.platform === 'win32')
                for (const prior of Object.keys(environment))
                    if (prior.toUpperCase() === identity)
                        delete environment[prior];
            environment[key] = value;
        }
    }
    return environment;
}
export function prepareWorkflowBinding(bindingValue, runtime) {
    const binding = parseWorkflowBinding(bindingValue);
    if (!runtime || binding.role === 'lead' || !binding.executableIdentity)
        throw new Error('workflow_runtime_required');
    if (binding.role === 'reviewer' && binding.capabilities.includes('review-permission-transition'))
        throw new Error('workflow_review_transition_unsupported');
    if (binding.providerRoute !== 'claude' && isExternalLLMDisabled())
        throw new Error('workflow_external_llm_disabled');
    let selected;
    try {
        selected = runtime.resolveBinding(binding);
    }
    catch {
        throw new Error('workflow_auth_profile_unavailable');
    }
    const profile = selected.authProfile;
    if (profile.ref !== binding.authProfileRef || profile.providerRoute !== binding.providerRoute)
        throw new Error('workflow_auth_route_mismatch');
    const environment = childEnvironment(profile);
    const captured = captureAuthProfile(profile);
    if (captured.fingerprint !== binding.authFingerprint)
        throw new Error('workflow_auth_profile_changed');
    const command = binding.executableIdentity.path;
    if (!isAbsolute(command) || process.platform === 'win32' && /\.(?:cmd|bat|ps1)$/i.test(command))
        throw new Error('workflow_shell_wrapper_unsupported');
    try {
        if (fileDigest(command) !== binding.executableIdentity.sha256)
            throw new Error('changed');
    }
    catch {
        throw new Error('workflow_executable_unavailable_or_changed');
    }
    ...
    if (evidence.validation !== 'authenticated' && !(evidence.validation === 'synthetic' && runtime.allowSyntheticCapabilities))
        throw new Error('workflow_capability_validation_required');
    ...
    if (evidence.dependencies !== undefined) {
        if (!Array.isArray(evidence.dependencies) || evidence.dependencies.length > 20)
            throw new Error('workflow_invalid_capability_evidence');
        for (const dependency of evidence.dependencies) {
            const entry = object(dependency);
            if (typeof entry.path !== 'string' || fileDigest(entry.path) !== entry.sha256)
                throw new Error('workflow_executable_dependency_changed');
        }
    }
    ...
    if (evidence.actorId !== undefined && (typeof evidence.actorId !== 'string' || !/^[A-Za-z0-9][A-Za-z0-9._:/@+-]{0,199}$/.test(evidence.actorId)))
        throw new Error('workflow_invalid_runner_identity');
    const secretValues = [...Object.entries(environment).filter(([key]) => /key|token|secret|password|credential|authorization/i.test(key)).map(([, value]) => value),
        ...Object.values(captured.redactionEnvironment)].filter((value) => value !== undefined);
...
export function workflowWorkerArguments(prepared, session) {
    const binding = prepared.binding;
    if (binding.role !== 'implementer')
        throw new Error('workflow_adapter_operation_mismatch');
...
export function workflowReviewerArguments(prepared, schemaFile, resultFile, schema) {
    const binding = prepared.binding;
    if (binding.role !== 'reviewer')
        throw new Error('workflow_adapter_operation_mismatch');
    if (binding.providerRoute === 'codex')
        return ['exec', '--sandbox', 'read-only', '--ephemeral', '--json', '--model', binding.model,
...
export function workflowReviewProvenance(prepared, head, runtime) {
    const reviewerId = prepared.actorId ?? `unknown:${prepared.binding.id}`;
    if (!runtime.reviewAuthorship || !prepared.actorId)
    ...
    if (authorIds.length > 100 || authorIds.some(id => typeof id !== 'string' || !/^[A-Za-z0-9][A-Za-z0-9._:/@+-]{0,199}$/.test(id)))
        throw new Error('workflow_invalid_runner_identity');
...
export function createClaudeWorkflowResultDecoder() {
    ...
    const line = (value) => {
        ...
        try {
            if (event.subtype !== 'success' || event.is_error !== false || !Object.hasOwn(event, 'structured_output')
                || Buffer.byteLength(JSON.stringify(event.structured_output) ?? '') > 64 * 1024) {
                invalid = true;
                return;
            }
    ...
    return {
        ...
        finish() {
            ...
            if (pending)
                line(pending);
            if (oversized || invalid || terminalCount !== 1)
                throw new Error('workflow_invalid_claude_result');
            return output;
        },
    };
}
//# sourceMappingURL=workflow-adapters.js.map