...
export function workflowPromptFingerprint(prompt) {
    return createHash('sha256').update(prompt).digest('hex');
}
function balancedPayload(state, entry) {
...
export function buildWorkflowPrompt(state, entry, resultFile) {
    ...
    if (Buffer.byteLength(prompt) > 384 * 1024)
        throw new Error('workflow_prompt_too_large');
    return prompt;
}
export function workflowContextFingerprint(state) {
...
export function workflowSessionFingerprint(state, entry, command, worktree) {
...
//# sourceMappingURL=workflow-prompt.js.map