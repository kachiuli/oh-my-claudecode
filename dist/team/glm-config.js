...
export function validateGlmCommand(command) {
...
export function getGlmConfig(config = {}, env = process.env) {
...
export function resolveGlmExecutable(command) {
    validateGlmCommand(command);
...
export function applyGlmProfile(config) {
    if (config.team?.profile === undefined)
        return config;
    if (config.team.profile !== 'claude-glm-codex')
        throw new Error('Unknown team.profile');
    const roleRouting = {
    ...
    for (const [role, spec] of Object.entries(overrides ?? {})) {
        if (!spec || typeof spec !== 'object' || Array.isArray(spec))
            throw new Error(`team.roleRouting.${role} must be an object`);
        ...
        if (role !== canonical && overrides && Object.prototype.hasOwnProperty.call(overrides, canonical))
            continue;
...
//# sourceMappingURL=glm-config.js.map