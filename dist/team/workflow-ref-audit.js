...
function snapshot(cwd, workflowName) {
    const refs = new Map();
    ...
    for (const line of output.split("\n")) {
        ...
        if (ref.startsWith(`refs/heads/omc-team/${workflowName}/`))
            continue;
...
function checkpointShape(ref) {
    return (/^refs\/codex\/turn-diffs\/checkpoints\/[A-Za-z0-9][A-Za-z0-9._/-]{0,239}$/.test(ref) &&
        !ref.includes("..") &&
        !ref.includes("//") &&
        !ref.includes("@{") &&
        !ref.endsWith(".") &&
...
export class WorkflowRefAudit {
    ...
    relatedToWorker(object, workerCommit) {
        if (object === this.baseline.rootHead)
            return false;
    ...
    allowsExternalCheckpoint(workers) {
        if (this.overflow || this.changes.size === 0)
            return false;
        if (this.previous.rootHead !== this.baseline.rootHead ||
            this.previous.rootTree !== this.baseline.rootTree)
            return false;
        const byWorker = new Map(workers.map((worker) => [worker.worker, worker.commitSha]));
        if ([...this.providerWorkers].some((worker) => !byWorker.get(worker)))
            return false;
        const workerCommits = new Set([...this.providerWorkers].map((worker) => byWorker.get(worker)));
        const workerTrees = new Set();
        ...
        for (const change of this.changes.values()) {
            ...
            if (!tree ||
                ...
                [...workerCommits].some((commit) => this.relatedToWorker(change.after, commit)))
                return false;
        }
        return true;
    }
    finalize(artifactPath, workers) {
...
//# sourceMappingURL=workflow-ref-audit.js.map