...
function git(cwd, args) {
    try {
        return execFileSync('git', args, { cwd, encoding: 'utf8', stdio: 'pipe', timeout: 30_000, windowsHide: true }).trim();
    }
    catch {
        throw new Error('workflow_git_operation_failed');
    }
}
...
function boundedJson(path, max = 64 * 1024) {
    if (lstatSync(path).isSymbolicLink() || statSync(path).size > max)
        throw new Error('workflow_artifact_invalid_or_oversized');
    try {
        return JSON.parse(readFileSync(path, 'utf8'));
    }
    catch {
        throw new Error('workflow_invalid_json');
    }
}
async function runWorkflowProvider(input, resultFile, transport) {
...
function assertLeader(state) {
    if (git(state.cwd, ['branch', '--show-current']) !== state.plan.integrationBranch)
        throw new Error('workflow_integration_branch_mismatch');
    if (!clean(state.cwd))
        throw new Error('workflow_integration_worktree_dirty');
    const head = git(state.cwd, ['rev-parse', 'HEAD']);
    if (head !== state.integrationHead)
        throw new Error('workflow_integration_head_changed');
    return head;
}
function assertLeadCaller() {
    if (process.env.OMC_TEAM_WORKER || process.env.OMC_TEAM_WORKER_NAME || process.env.OMC_TEAM_WORKTREE_PATH)
        throw new Error('workflow_lead_authority_required');
}
function assertMutable(state) {
    if (state.stage === 'complete')
        throw new Error('workflow_already_complete');
}
function save(state) {
    state.updatedAt = now();
    const previous = savedSnapshots.get(state);
    if (previous)
        validateWorkflowStateTransition(previous, state);
    else
...
export function readWorkflow(cwd, name) {
    let state = boundedJson(statePath(cwd, name), 16 * 1024 * 1024);
    if (state?.schemaVersion === 1 || state?.schemaVersion === 2)
        state = parseWorkflowState(state);
    if (!state || !((state.schemaVersion === 1 && state.profile === 'claude-glm-codex') || (state.schemaVersion === 2 && state.profile === 'role-substitution')) || state.plan?.name !== name
        || realpathSync(state.cwd) !== realpathSync(cwd))
        throw new Error('workflow_invalid_state');
    if (!Array.isArray(state.tasks) || !Array.isArray(state.reviews))
        throw new Error('workflow_invalid_state');
    if (state.options.mode !== undefined && !['v1', 'balanced'].includes(state.options.mode))
        throw new Error('workflow_invalid_mode');
...
async function mutate(cwd, name, action) {
    assertLeadCaller();
...
function count(value, fallback, max, min = 1) {
...
const NON_RETRYABLE_WORKER_ERRORS = ['workflow_timeout', 'workflow_interrupted', 'workflow_output_incomplete',
    ...
    'workflow_protected_ref_audit_failed', 'workflow_session_identity_mismatch'];
...
export async function initWorkflowV2(cwd, rawPlan, bindings, options = {}) {
    if (options.mode !== undefined && options.mode !== 'balanced')
        throw new Error('workflow_balanced_mode_required');
    ...
    for (const role of ['lead', 'implementer', 'reviewer'])
        if (selected[role].role !== role)
            throw new Error('workflow_binding_role_mismatch');
    const state = await initializeWorkflow(cwd, rawPlan, { ...options, mode: 'balanced' }, selected);
    if (state.schemaVersion !== 2)
        throw new Error('workflow_invalid_state');
    return state;
}
async function initializeWorkflow(cwd, rawPlan, options, bindings) {
    assertLeadCaller();
    return withOrchestratorOperation(cwd, async () => {
        if (redactWorkflowText(JSON.stringify({ rawPlan, options })) !== JSON.stringify({ rawPlan, options }))
            throw new Error('workflow_sensitive_input_rejected');
        cwd = realpathSync(cwd);
        const plan = parseWorkflowPlan(rawPlan);
        if (options.mode !== undefined && !['v1', 'balanced'].includes(options.mode))
            throw new Error('workflow_invalid_mode');
        ...
        if (existsSync(teamStateRoot(cwd, plan.name)))
            throw new Error('workflow_name_already_exists');
        if (!clean(cwd))
            throw new Error('workflow_leader_worktree_dirty');
        if (/^(?:main|master)$/i.test(plan.integrationBranch) || plan.integrationBranch.startsWith('-'))
            throw new Error('workflow_integration_branch_required');
        git(cwd, ['check-ref-format', '--branch', plan.integrationBranch]);
        if (git(cwd, ['rev-parse', `${plan.baseCommit}^{commit}`]) !== plan.baseCommit)
            throw new Error('workflow_invalid_base');
        for (const task of plan.tasks)
            if (task.baseCommit !== plan.baseCommit)
                throw new Error('workflow_task_base_mismatch');
        ...
        if (!bindings && (executor.provider !== 'glm' || reviewer.provider !== 'codex'))
        ...
        if (redactWorkflowText(JSON.stringify(resolvedOptions)) !== JSON.stringify(resolvedOptions))
            throw new Error('workflow_sensitive_input_rejected');
        // Resolve lazily at dispatch: init/status remain usable when a provider has not been installed yet.
        boundedText(resolvedOptions.glmCommand, 1000);
        boundedText(resolvedOptions.codexCommand, 1000);
        if (resolvedOptions.glmModel !== undefined)
            boundedText(resolvedOptions.glmModel, 160);
        if (resolvedOptions.codexModel !== undefined)
            boundedText(resolvedOptions.codexModel, 160);
        if (git(cwd, ['branch', '--show-current']) !== plan.integrationBranch) {
            git(cwd, ['switch', '-c', plan.integrationBranch, plan.baseCommit]);
        }
        else if (git(cwd, ['rev-parse', 'HEAD']) !== plan.baseCommit)
            throw new Error('workflow_initial_head_mismatch');
        const legacy = {
        ...
        const state = bindings ? { ...legacy, schemaVersion: 2, profile: 'role-substitution', bindings, substitutions: [],
            tasks: plan.tasks.map((task, index) => ({ task, canonicalId: String(index + 1), status: 'pending', attempts: 0,
...
function getTask(state, id) {
    ...
    if (!task)
        throw new Error('workflow_task_not_found');
    return task;
}
function artifactsRoot(state) {
 ...
 */
function providerTimeoutMs(state) {
    return state.options.providerPolicy === 'supervised' ? null : state.options.timeoutMs;
}
function prepareBinding(state, binding, runtime) {
    try {
        return prepareWorkflowBinding(binding, runtime);
    }
    catch (error) {
...
export async function substituteWorkflowBinding(cwd, name, input) {
    return mutate(cwd, name, async (state) => {
        if (state.schemaVersion !== 2)
            throw new Error('workflow_role_substitution_profile_required');
        if (assertLeader(state) !== input.expectedHead)
            throw new Error('workflow_substitution_head_mismatch');
        if (state.tasks.some(entry => entry.status === 'running'))
        ...
        if (task && !['pending', 'failed'].includes(task.status))
            throw new Error('workflow_substitution_task_not_pending');
...
function assertWorker(state, entry) {
    const expected = getWorktreePath(state.cwd, state.plan.name, entry.worker);
    if (!entry.worktree || resolve(entry.worktree) !== resolve(expected) || entry.branch !== getBranchName(state.plan.name, entry.worker))
        throw new Error('workflow_worker_mapping_mismatch');
    validateResolvedPath(expected, join(teamStateRoot(state.cwd, ''), '..', '..', 'team'));
    if (git(expected, ['branch', '--show-current']) !== entry.branch)
        throw new Error('workflow_worker_branch_mismatch');
    ...
    if (!canonicalExpected || expandPathForCompare(git(expected, ['rev-parse', '--show-toplevel'])) !== canonicalExpected)
        throw new Error('workflow_worker_mapping_mismatch');
    const registered = git(state.cwd, ['worktree', 'list', '--porcelain']);
    if (!registered.split('\n').some(line => line.startsWith('worktree ') && expandPathForCompare(line.slice('worktree '.length)) === canonicalExpected))
        throw new Error('workflow_worker_unregistered');
    if (!clean(expected))
        throw new Error('workflow_worker_worktree_dirty');
    return expected;
}
function validateCommit(state, entry) {
    const cwd = assertWorker(state, entry);
    const sha = entry.handoff?.commitSha;
    if (!sha || git(cwd, ['rev-parse', 'HEAD']) !== sha)
        throw new Error('workflow_worker_commit_mismatch');
    const parents = git(cwd, ['rev-list', '--parents', '-n', '1', sha]).split(' ');
    if (parents.length !== 2 || parents[1] !== entry.task.baseCommit)
        throw new Error('workflow_worker_single_commit_required');
    ...
    if (!files.length || files.length > 100)
        throw new Error('workflow_invalid_changed_files');
    for (const file of files)
        if (!matchesScope(file, entry.task.writeScope) || matchesScope(file, entry.task.prohibitedScope))
            throw new Error('workflow_out_of_scope_changes');
    if (JSON.stringify([...files].sort()) !== JSON.stringify([...(entry.handoff?.changedFiles ?? [])].sort()))
        throw new Error('workflow_changed_files_mismatch');
    return files;
}
function sessionFingerprint(state, entry, command, worktree) {
...
async function executeTask(state, entry, command, resumeReason, prepared, refAudit, orchestrationHost = 'claude') {
    ...
    while (entry.attempts < state.options.maxAttempts) {
        ...
        try {
            ...
            if (!worktree)
                throw new Error('workflow_worktree_required');
            entry.worktree = worktree.path;
            entry.branch = worktree.branch;
            assertWorker(state, entry);
            if (git(entry.worktree, ['rev-parse', 'HEAD']) !== entry.task.baseCommit)
                throw new Error('workflow_worker_base_mismatch');
            save(state);
            ...
            if (sessionEnabled) {
                ...
                if (resuming && (!entry.session?.confirmed || entry.session.fingerprint !== fingerprint))
                    throw new Error('workflow_session_identity_changed');
            ...
            try {
                result = await runWorkflowProvider({ command, args: prepared ? workflowWorkerArguments(prepared, sessionEnabled ? { id: entry.session.id, resume: resuming } : undefined) : [...buildLaunchArgs('glm', {
                    ...
                    ...(prepared ? { environment: prepared.environment, redactionEnvironment: prepared.redactionEnvironment } : {}) }, resultFile, { kind: 'worker-designated-result', taskId: entry.task.id });
            }
            finally {
                try {
                    publication.revoke();
                    ...
                    refAudit?.providerCompleted(entry.worker);
                }
            }
            ...
            if (handoff.outcome !== 'completed' || handoff.tests.some(test => !test.passed))
                throw new Error('workflow_worker_reported_failure');
            for (const test of entry.task.tests)
                if (!handoff.tests.some(result => result.passed && result.command === test.command && JSON.stringify(result.args) === JSON.stringify(test.args)))
                    throw new Error('workflow_worker_test_evidence_missing');
            ...
            try {
                for (const [index, test] of entry.task.tests.entries()) {
                    ...
                    if (!check.passed)
                        throw new Error('workflow_worker_test_failed');
                }
            }
            finally {
                refAudit?.controllerReplayCompleted();
            }
            validateCommit(state, entry);
            ...
            if (nonRetryableWorkerError(entry.error))
                break;
            // Any work or moved HEAD is retained for the lead; retry only pristine attempts.
            if (entry.worktree) {
                try {
                    if (!clean(entry.worktree) || git(entry.worktree, ['rev-parse', 'HEAD']) !== entry.task.baseCommit)
                        break;
                }
                catch {
                    break;
            ...
            if (entry.attempts < state.options.maxAttempts) {
                ...
                entry.backoffUntil = new Date(Date.now() + delay).toISOString();
                save(state);
            ...
            if (state.schemaVersion === 1) {
                ...
                if (!invocation.telemetry.durationMs)
                    invocation.telemetry.durationMs = Date.now() - started;
            }
            delete entry.claimToken;
            delete entry.backoffUntil;
            entry.updatedAt = now();
            save(state);
        }
    }
}
function attachRefAudit(entries, artifact) {
    if (!artifact)
        return;
    for (const entry of entries) {
        if (entry.handoff && !entry.handoff.artifacts.some(item => item.path === artifact.path))
            entry.handoff.artifacts.push(artifact);
        const invocation = entry.invocations?.at(-1);
        if (invocation?.error === 'workflow_invocation_incomplete' && !invocation.artifacts.some(item => item.path === artifact.path))
            invocation.artifacts.push(artifact);
    }
}
function failProtectedRefs(entries, balanced, onlyIncomplete = false, error = 'workflow_protected_refs_changed') {
    for (const entry of entries)
        if (balanced || entry.status === 'completed') {
            entry.status = 'failed';
            entry.error = error;
            if (entry.session)
                entry.session.confirmed = false;
            const invocation = entry.invocations?.at(-1);
            if (invocation && (!onlyIncomplete || invocation.error === 'workflow_invocation_incomplete')) {
                invocation.outcome = 'failed';
                invocation.error = entry.error;
            }
        }
}
function settleVersionedInvocations(state, entries) {
    if (state.schemaVersion !== 2)
        return;
    for (const entry of entries) {
        const invocation = entry.invocations?.at(-1);
        if (invocation?.error !== 'workflow_invocation_incomplete')
            continue;
        invocation.outcome = entry.status === 'completed' ? 'completed' : 'failed';
        if (entry.error)
            invocation.error = entry.error;
...
export async function runWorkflow(cwd, name, runtime) {
    return mutate(cwd, name, async (state, active) => {
        ...
        if (!candidates.length) {
            if (state.schemaVersion === 2 && state.tasks.some(entry => entry.status === 'failed' && entry.attempts >= state.options.maxAttempts))
                throw new Error('workflow_attempt_limit_reached');
            return;
        }
        let command = '';
        if (state.schemaVersion === 1) {
            try {
                command = resolveGlmExecutable(state.options.glmCommand);
            }
            catch {
                throw new Error('workflow_glm_unavailable_fallback_disabled');
            }
        }
        let cursor = 0;
        ...
        if (state.schemaVersion === 1 && pools.some(pool => pool.status === 'rejected'))
            throw new Error('workflow_worker_persistence_failed');
        try {
            let audit;
            try {
                audit = refAudit.finalize(() => join(artifactsRoot(state), `ref-audit-${randomUUID()}.json`), candidates.map(entry => ({ worker: entry.worker, commitSha: entry.handoff?.commitSha })));
            }
            catch {
...
export async function resumeWorkflowTask(cwd, name, taskId, expectedHead, reason, runtime) {
    return mutate(cwd, name, async (state, active) => {
        if (state.options.mode !== 'balanced')
            throw new Error('workflow_balanced_mode_required');
        if (assertLeader(state) !== expectedHead)
            throw new Error('workflow_resume_head_mismatch');
        if (state.tasks.some(entry => entry.status === 'running'))
            throw new Error('workflow_interrupted_worker_requires_inspection');
        const safeReason = redactWorkflowText(boundedText(reason, 1000));
        const entry = getTask(state, taskId);
        if (entry.status !== 'failed')
            throw new Error('workflow_resume_failed_task_required');
        if (entry.attempts >= state.options.maxAttempts)
            throw new Error('workflow_attempt_limit_reached');
        if (state.schemaVersion === 1 && !state.options.glmModel)
            throw new Error('workflow_resume_model_required');
        ...
        if (['workflow_worker_modified_protected_refs', 'workflow_protected_refs_changed', 'workflow_protected_ref_audit_failed',
            'workflow_session_identity_mismatch'].includes(entry.error ?? ''))
            throw new Error('workflow_session_not_resumable');
        const session = entry.session;
        if (!session?.confirmed || !/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(session.id))
            throw new Error('workflow_session_not_confirmed');
        const worktree = realpathSync(assertWorker(state, entry));
        if (session.worktree !== worktree || session.branch !== entry.branch)
            throw new Error('workflow_session_identity_changed');
        if (git(worktree, ['rev-parse', 'HEAD']) !== entry.task.baseCommit)
            throw new Error('workflow_worker_base_mismatch');
        const prepared = state.schemaVersion === 2 ? prepareBinding(state, state.bindings.implementer, runtime) : undefined;
        let command;
        if (prepared) {
            if (!('binding' in session))
                throw new Error('workflow_session_not_confirmed');
        ...
        try {
            await executeTask(state, entry, command, safeReason, prepared, refAudit, active.host);
        }
        catch (error) {
        ...
        try {
            let audit;
            try {
                audit = refAudit.finalize(() => join(artifactsRoot(state), `ref-audit-${randomUUID()}.json`), [{ worker: entry.worker, commitSha: entry.handoff?.commitSha }]);
            }
            catch {
            ...
            settleVersionedInvocations(state, [entry]);
        }
        if (executionFailure)
...
export async function acceptWorkflowTask(cwd, name, taskId) {
    return mutate(cwd, name, async (state) => {
        assertLeader(state);
        const entry = getTask(state, taskId);
        if (entry.status !== 'completed')
            throw new Error('workflow_task_not_awaiting_acceptance');
        if (!entry.task.dependencies.every(id => getTask(state, id).status === 'accepted'))
            throw new Error('workflow_dependency_not_integrated');
        validateCommit(state, entry);
...
function integrated(state) {
    if (state.tasks.some(entry => !['accepted', 'rejected'].includes(entry.status)) || !state.tasks.some(entry => entry.status === 'accepted'))
        throw new Error('workflow_integration_incomplete');
}
export async function verifyWorkflow(cwd, name) {
    return mutate(cwd, name, async (state) => {
        integrated(state);
        ...
        if (assertLeader(state) !== head)
            state.verification.passed = false;
    });
}
function verificationGate(state) {
    integrated(state);
    const head = assertLeader(state);
    if (!state.verification?.passed || state.verification.head !== head)
        throw new Error('workflow_current_verification_required');
    return head;
}
/** Review locations are metadata; retain raw artifacts and validate the relative copy with the normal scope guard. */
export function parseWorkflowReviewFindings(value, pass, cwd) {
    ...
    const relativeFile = (file) => {
        ...
        if (!absolute || slash.startsWith('//') || (paths === win32 ? slash.slice(3) : slash.slice(1))
            ...
            return file;
        if (!inside(cwd, file) || !paths.relative(cwd, file))
            return file;
        try {
            ...
            for (;;) {
                try {
                    if (!inside(canonicalRoot, realpathSync(ancestor)))
                        return file;
                    break;
                }
                catch (error) {
    ...
    return parseWorkflowFindings({ ...value, findings: value.findings.map(finding => finding && typeof finding === 'object' && typeof finding.file === 'string'
            ? { ...finding, file: relativeFile(finding.file) } : finding) }, pass);
}
function reviewContext(state) {
    ...
    for (const path of affected) {
        let scope = path;
        while (scope !== '.') {
            paths.add(posix.join(scope, 'AGENTS.md'));
            paths.add(posix.join(scope, 'CLAUDE.md'));
    ...
    const projectInstructions = [...paths].filter(path => existsSync(join(state.cwd, path))).sort().map(path => {
        ...
        validateResolvedPath(file, state.cwd);
        if (!lstatSync(file).isFile() || lstatSync(file).isSymbolicLink() || statSync(file).size > 64 * 1024)
...
export async function reviewWorkflow(cwd, name, runtime) {
    return mutate(cwd, name, async (state, active) => {
        const head = verificationGate(state);
        const refs = git(cwd, ['for-each-ref', '--format=%(refname) %(objectname)']);
        if (state.reviewPasses >= state.options.maxReviewPasses)
            throw new Error('workflow_review_limit_reached');
        if (state.reviews.some(review => review.findings.some(finding => !finding.disposition || finding.disposition === 'fix' && !finding.fixedBy)))
            throw new Error('workflow_findings_require_adjudication_or_fix');
        ...
        state.reviewPasses++;
        state.stage = 'review';
        const reservation = { orchestrationHost: active.host, pass: state.reviewPasses, head,
        ...
        if (state.schemaVersion === 2 && prepared && provenance) {
            const bound = { ...reservation, invocationId: randomUUID(), binding: prepared.binding, provenance };
            (state.reviewAttempts ??= []).push(bound);
            attempt = bound;
        }
        else if (state.schemaVersion === 1) {
            (state.reviewAttempts ??= []).push(reservation);
            attempt = reservation;
        }
        save(state);
        try {
            const root = artifactsRoot(state);
            const prefix = join(root, `review-${state.reviewPasses}`);
            const resultFile = `${prefix}.result.json`;
            const schemaFile = `${prefix}.schema.json`;
            ...
            try {
                if (assertLeader(state) !== head || git(cwd, ['for-each-ref', '--format=%(refname) %(objectname)']) !== refs)
                    throw new Error('changed');
            }
            catch {
                throw new Error('workflow_reviewer_modified_repository');
            }
            if (!result.passed)
                throw new Error('workflow_review_process_failed');
            if (result.outputError)
                throw result.outputError;
            validateResolvedPath(result.outputArtifactPath, root);
            ...
            if (attempt) {
                ...
                attempt.artifacts = state.reviews.at(-1).artifacts;
            }
        }
        catch (error) {
            if (attempt) {
            ...
            if (attempt && !attempt.telemetry.durationMs)
                attempt.telemetry.durationMs = Date.now() - started;
        }
    });
}
export async function adjudicateWorkflow(cwd, name, decisions) {
    return mutate(cwd, name, async (state) => {
        const review = state.reviews.at(-1);
        if (!review || !Array.isArray(decisions) || decisions.length > 50)
            throw new Error('workflow_review_required');
        const seen = new Set();
        const updates = [];
        for (const decision of decisions) {
            const finding = review.findings.find(finding => finding.id === decision.findingId);
            if (!finding || finding.disposition || seen.has(finding.id) || !['fix', 'dismiss'].includes(decision.disposition))
                throw new Error('workflow_invalid_disposition');
            seen.add(finding.id);
            updates.push({ finding, disposition: decision.disposition, reason: redactWorkflowText(boundedText(decision.reason, 1000)) });
        }
        for (const update of updates) {
            update.finding.disposition = update.disposition;
            update.finding.reason = update.reason;
        }
        state.stage = updates.some(update => update.disposition === 'fix') ? 'remediation' : 'adjudication';
    });
}
export async function addWorkflowFix(cwd, name, rawTask, findingIds) {
    return mutate(cwd, name, async (state) => {
        const head = assertLeader(state);
        const task = parseWorkflowTask(rawTask);
        if (redactWorkflowText(JSON.stringify(task)) !== JSON.stringify(task))
            throw new Error('workflow_sensitive_input_rejected');
        if (state.tasks.some(entry => entry.task.id === task.id) || task.baseCommit !== head || !Array.isArray(findingIds) || !findingIds.length)
            throw new Error('workflow_invalid_fix');
        ...
        if (new Set(findingIds).size !== findingIds.length || findingIds.some(id => !findings.some(finding => finding.id === id && finding.disposition === 'fix' && !finding.fixedBy))
            || state.tasks.some(entry => entry.status !== 'rejected' && entry.findingIds?.some(id => findingIds.includes(id))))
            throw new Error('workflow_fix_finding_not_actionable');
...
export async function finishWorkflow(cwd, name) {
    return mutate(cwd, name, async (state) => {
        verificationGate(state);
        if (!state.reviews.length)
            throw new Error('workflow_review_required');
        const findings = state.reviews.flatMap(review => review.findings);
        if (findings.some(finding => !finding.disposition || finding.disposition === 'fix' && !finding.fixedBy))
            throw new Error('workflow_unresolved_findings');
        state.stage = 'complete';
    });
}
export function workflowStatus(cwd, name) {
    const state = readWorkflow(cwd, name);
...
export async function cleanupWorkflow(cwd, name) {
    assertLeadCaller();
    return withOrchestratorOperation(cwd, () => withFileLock(`${statePath(cwd, name)}.lock`, async () => {
        const state = readWorkflow(cwd, name);
        if (state.stage !== 'complete')
            throw new Error('workflow_completion_required_for_cleanup');
        assertLeader(state);
        ...
        for (const entry of state.tasks) {
            ...
            if (entry.status !== 'accepted') {
                result.preserved.push({ taskId: entry.task.id, reason: 'unaccepted_work_preserved' });
                continue;
            }
            try {
                validateCommit(state, entry);
                ...
                if (existsSync(entry.worktree))
                    throw new Error('workflow_cleanup_unverified');
                result.removed.push(entry.task.id);
            }
            catch {
                result.preserved.push({ taskId: entry.task.id, reason: 'dirty_changed_or_unverified_worktree_preserved' });
            }
        }
