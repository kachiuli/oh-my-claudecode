...
import { awaitWorkerLaunchAcknowledgement, awaitWorkerLaunchProviderStarted, buildWorkerLaunchBootstrapSpec, prepareWorkerLaunchAttempt, runWorkerLaunchBootstrap, terminateWorkerLaunchProvider, } from '../worker-launch-ack.js';
...
function fixtureTeamRoot(cwd, teamName) {
...
describe('shutdownTeamV2 detached worktree cleanup', () => {
    ...
    beforeEach(() => {
        ...
        tmuxMocks.getOwnedWorkerLiveness.mockImplementation(async (ownership) => {
            if (ownership.provider === 'tmux' && !ownership.tmuxServerIdentity)
                return 'unknown';
    ...
    it('commits the stopped fence when the post-cleanup Ralph summary cannot read tasks', async () => {
        ...
        const stoppedConfig = JSON.parse(readFileSync(configPath, 'utf8'));
    ...
    it('keeps a queued second shutdown behind final instance disposal', async () => {
        ...
        let releaseDispose;
        let enteredDispose;
        const disposeEntered = new Promise(resolveEntered => { enteredDispose = resolveEntered; });
        const disposeGate = new Promise(resolveGate => { releaseDispose = resolveGate; });
        ...
        try {
            ...
            const firstSettled = first.then(value => ({ status: 'resolved', value }), error => ({ status: 'rejected', error }));
            firstCompletion = firstSettled;
            const gateOrEarlyExit = await Promise.race([
                disposeEntered.then(() => ({ status: 'entered' })),
                firstSettled,
            ...
            if (gateOrEarlyExit.status === 'rejected')
                throw gateOrEarlyExit.error;
            if (gateOrEarlyExit.status === 'timeout')
                throw new Error('final disposal gate was not reached');
