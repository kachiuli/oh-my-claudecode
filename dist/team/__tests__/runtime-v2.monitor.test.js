...
function fixtureProcessStartIdentity() {
...
const mocks = vi.hoisted(() => {
    ...
    const getWorkerLiveness = vi.fn(async (_paneId) => 'alive');
    return {
        tmuxServerIdentity,
        getWorkerLiveness,
...
describe('monitorTeamV2 pane-based stall inference', () => {
    ...
    async function writeConfigAndTask(taskStatus = 'pending', instanceId = TEAM_INSTANCE_ID, reserve = true) {
        if (reserve)
            await reserveTeamInstance({ teamName: 'demo-team', cwd, instanceId });
    ...
    it('reports provider death independently when its pane transport remains alive', async () => {
        ...
        const expected = JSON.parse(await readFile(attempt.expectedPath, 'utf8'));
        ...
        const config = JSON.parse(await readFile(join(teamRoot, 'config.json'), 'utf8'));
        config.workers[0] = {
            ...config.workers[0],
            pane_id: '%2',
            worker_cli: 'claude',
    ...
    it('aborts a stale expected-instance cycle before mutating the replacement', async () => {
        ...
        let releaseLiveness;
        let livenessStarted;
        const livenessGate = new Promise(resolveGate => { releaseLiveness = resolveGate; });
        const scanStarted = new Promise(resolveStarted => { livenessStarted = resolveStarted; });
        ...
        await scanStarted;
        await disposeTeamInstance(createTeamInstanceBinding({ teamName: 'demo-team', cwd, instanceId: TEAM_INSTANCE_ID }), FINAL_DISPOSAL_AUTHORIZATION);
