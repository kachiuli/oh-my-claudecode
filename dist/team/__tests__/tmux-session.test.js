...
import { sanitizeName, sessionName, shouldAttemptAdaptiveRetry, getDefaultShell, buildWorkerStartCommand, paneLooksReady, paneHasActiveTask, paneHasTrustPrompt, verifyTeamTargetOwnership, TeamSessionCreationError, normalizeDetachedSessionTarget, buildDetachedTmuxServerKeepaliveArgs, buildPrivateTmuxSocketPath, captureTmuxServerIdentity, observeTmuxServerIdentity, runTmuxServerIdentityGuard, } from '../tmux-session.js';
...
describe('tmux creation allocation evidence', () => {
    it('retains typed evidence when creation cleanup is unknown', () => {
        const error = new TeamSessionCreationError('tmux_creation_cleanup_unverified', {
            sessionName: 'team:0',
            leaderPaneId: '%1',
            workerPaneIds: [],
...
describe('verifyTeamTargetOwnership tmux target kinds', () => {
    ...
    const dependenciesFor = (tmuxExec) => ({
        tmuxExec,
...
describe('tmux server incarnation identity', () => {
    ...
    ])('keeps process identity observation distinct for %s', async (_label, processState, expected) => {
    ...
    it('rejects malformed identity before querying and rejects a mismatched server response', async () => {
        ...
        expect(tmuxQuery).toHaveBeenCalledExactlyOnceWith(['-S', identity.socket_path, 'display-message', '-p', '#{pid}'], { timeout: 2_000, stripTmux: true });
    });
...
describe('getWorkerLiveness tmux inventory fallback', () => {
    async function loadLivenessWithTmuxOutputs(displayOutput, inventoryOutput) {
        const calls = [];
        const tmuxCmdAsync = vi.fn(async (args) => {
            calls.push(args);
            if (args[0] === 'display-message')
                return { stdout: displayOutput, stderr: '' };
    ...
    it.each(['', 'not-a-pane\n', '%0 malformed\n', '%0 0\nmalformed\n'])('preserves unknown when fallback inventory is empty or malformed: %j', async (inventoryOutput) => {
