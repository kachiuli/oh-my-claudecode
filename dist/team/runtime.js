...
import { adoptWorkerPaneOwnership, observeTmuxServerIdentity, isWorkerAlive, verifyTeamTargetOwnership, } from './tmux-session.js';
 ...
 */
export async function startTeam(_config) {
 ...
 * pane IDs.
 */
export async function shutdownTeam(teamName, sessionName, cwd, timeoutMs = 30_000, workerPaneIds, leaderPaneId, ownsWindow, instanceId) {
...
export async function resumeTeam(teamName, cwd) {
    ...
    if (!configData || !isValidTeamInstanceId(configData.instance_id))
        return null;
    ...
    if (workerPaneEntries.some(([name, paneId]) => (!workerNames.includes(name)
        || typeof paneId !== 'string'
        || paneId.trim().length === 0)))
        return null;
    const workerPaneIds = workerPaneEntries.map(([, paneId]) => paneId);
    if (new Set(workerPaneIds).size !== workerPaneIds.length)
        return null;
    if (Array.isArray(configData.workerPaneIds)) {
        if (new Set(configData.workerPaneIds).size !== configData.workerPaneIds.length
            ...
            || configData.workerPaneIds.some(paneId => !workerPaneIds.includes(paneId)))
            return null;
    }
    ...
    if (leaderOwnership.kind !== 'owned')
        return null;
    const workerPaneOwnership = new Map();
    for (let i = 0; i < workerPaneIds.length; i++) {
        const paneId = workerPaneIds[i];
    ...
    for (let i = 0; i < configData.tasks.length; i++) {
        ...
        if (task?.status === 'in_progress' && task.owner) {
            ...
            if (!paneId || !workerPaneOwnership.has(paneId))
                return null;
