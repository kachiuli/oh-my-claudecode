...
- `omc team workflow run`, `resume`, `verify` and `review` drive providers and can take several minutes. Give each call the full provider timeout (for example a 600000 ms Bash tool timeout in Claude Code, or a yielding `exec` with polling in Codex); never background, kill or retry them. If a lead dies mid-attempt, run `omc orchestrator recover`, then settle the interrupted task with `omc team workflow reject <name> <task> --reason <text>` before continuing.
