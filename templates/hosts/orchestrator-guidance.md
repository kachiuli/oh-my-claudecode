...
- `omc team workflow run`, `resume`, `verify` and `review` drive providers and can take as long as the workflow's provider timeout. Give each call at least that long (a Claude Code Bash `timeout` up to the launch-provided maximum, or a yielding `exec` with polling in Codex); never background, kill or retry them. If a lead dies mid-attempt, exit that lead, run `omc orchestrator recover` from a plain shell in the checkout, then settle the interrupted task with `omc team workflow reject <name> <task> --reason <text>` before starting a new lead.
