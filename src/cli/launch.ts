...
export function extractOmcLaunchOptions(args: string[]): { args: string[]; environment: Record<string, string> } {
  ...
  if (!notify.notifyEnabled) environment.OMC_NOTIFY = '0';
  const toggles = [
  ...
  for (const [key, enabled] of toggles) {
    if (enabled !== undefined) environment[key] = enabled ? '1' : '0';
  }
