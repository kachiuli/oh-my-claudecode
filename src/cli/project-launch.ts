...
export function projectHostEnvironment(
...
function launchArguments(
  host: OrchestratorHost,
  args: string[],
): { args: string[]; resumeId?: string } {
  const remaining: string[] = [];
  let resumeId: string | undefined;
  for (let index = 0; index < args.length; index++) {
    const argument = args[index];
    if (
      /^--(?:remote|remote-workspace|cloud|environment|teleport|from-pr)(?:=|$)/.test(
        argument,
      ) ||
    ...
    if (
      host === "claude" &&
      (/^--(?:safe-mode|bare|allow-dangerously-skip-permissions|bg|background)(?:=|$)/.test(
        argument,
      ) ||
      ...
      throw new Error(
        "orchestrator_incompatible_native_mode: keep project integration and native permission checks enabled.",
      );
    }
    if (
      /^--(?:madmax|yolo|dangerously-skip-permissions)(?:=|$)/.test(argument)
    ) {
      throw new Error(
        "orchestrator_unsafe_permission_alias: project launches retain native permission checks; remove the bypass flag.",
      );
    }
    if (argument === "--resume") {
      ...
      if (
        /^(?:resume|fork|--resume=.*|--continue(?:=.*)?|--last|--session-id(?:=.*)?)$/.test(
          argument,
        ) ||
        (host === "claude" && /^-[rc]/.test(argument))
      ) {
        throw new Error(
          "Use omc launch --resume <registered-native-session-id>; sessions from another host or selection are refused.",
        );
      }
      if (
        /^(?:--cd(?:=.*)?|--worktree(?:=.*)?)$/.test(argument) ||
        /^-(?:C|w)/.test(argument)
      ) {
        throw new Error(
          "Launch from the intended repository; the shared OMC controller owns workflow worktrees.",
        );
      }
...
async function runNativeHost(
  ...
  return new Promise((resolve, reject) => {
    ...
    child.once("error", () => {
      ...
      reject(
        new Error(
          "orchestrator_launch_failed: verify the selected native CLI executable.",
        ),
      );
    });
...
export async function launchProjectOrchestrator(
  args: string[],
  options: ProjectLaunchOptions = {},
): Promise<boolean> {
  const cwd = options.cwd ?? process.cwd();
  if (!readOrchestratorRepositoryConfig(cwd)) return false;
  if (
  ...
  if (
    ...
    throw new Error(
      `orchestrator_native_executable_required: install a directly executable ${active.host} CLI.`,
    );
  }
  ...
  try {
    ...
    const hostArgs = buildHostLaunchArgs(
      active.host,
      repositoryRoot,
      requested.args,
      requested.resumeId,
    ...
    const code = await (options.run ?? runNativeHost)(cli.path, hostArgs, {
      ...
      registerProcess: (pid) =>
        registerOrchestratorLeaseProcess(repositoryRoot, lease, pid),
    });
