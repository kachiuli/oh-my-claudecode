...
export function selectWindowsExecutableCandidate(
...
function isAbsoluteCmdExe(candidate: string): boolean {
...
export function validatedComspec(): string | undefined {
  try {
    if (!existsSync(SYSTEM_COMMAND_PROCESSOR)) return undefined;
    const canonical = realpathSync(SYSTEM_COMMAND_PROCESSOR);
    if (!isAbsoluteCmdExe(canonical)) return undefined;
...
export function resolveWindowsBatchInvocation(command: string, args: string[]): WindowsBatchInvocation {
