...
import {
  ...
  type OrchestratorRepositoryConfig,
...
import {
  readNativeHookDiagnostics,
  type NativeHookDiagnostics,
...
const HOST_ORDER: readonly OrchestratorHost[] = ["claude", "codex"];
...
export interface HostAssetDoctorEntry {
  ...
  readonly hookDiagnostics: NativeHookDiagnostics;
...
function codexMcpBlock(packageRoot: string, marketplaceName: string): string {
  return [
    MCP_START,
    "[mcp_servers.omc]",
    'command = "node"',
...
function nativeHookEntries(host: OrchestratorHost): HookMap {
  const command = `omc orchestrator hook --host ${host}`;
  const commandWindows = `omc.cmd orchestrator hook --host ${host}`;
  const handler = (timeout: number): Record<string, unknown> => ({
    type: "command",
    command,
...
function codexAgentToml(source: string, fallbackName: string): string {
  const name = frontmatterValue(source, "name") ?? fallbackName;
  const description =
    frontmatterValue(source, "description") ?? `OMC ${name} role`;
...
function packageVersion(packageRoot: string): string {
  try {
    const raw = JSON.parse(
      readSource(join(packageRoot, "package.json"), 128 * 1024),
    ) as { version?: unknown };
...
function skillContent(guidance: string): string {
  return [
    "---",
...
function hostAssets(
  ...
  packageRoot: string,
  guidance: string,
  previous: HostReceipt | undefined,
): DesiredAsset[] {
  const prior = new Map(
    previous?.assets.map((asset) => [asset.path, asset]) ?? [],
  );
  ...
  if (host === "claude") {
    ...
    const current =
      readTarget(root, join(root, instructionsPath)).bytes?.toString("utf8") ??
      "";
    desired.push({
  ...
  const marketplacePath = portablePath(
    join(".agents", "plugins", "marketplace.json"),
  );
  const currentMarketplace =
    readTarget(root, join(root, marketplacePath)).bytes?.toString("utf8") ?? "";
  const marketplace = mergedCodexMarketplace(
    currentMarketplace,
    prior.get(marketplacePath),
    {
      name: "omc-project-host",
  ...
  const currentConfig =
    readTarget(root, join(root, configPath)).bytes?.toString("utf8") ?? "";
  ...
  if (hasTomlTableInsertionConflict(unownedConfig, ["plugins", pluginKey])) {
    throw new Error(
      `host_assets_plugin_config_collision: omc-project-host@${marketplace.marketplaceName}`,
    );
  }
  desired.push({
    path: configPath,
  ...
  const currentHooks =
    readTarget(root, join(root, hooksPath)).bytes?.toString("utf8") ?? "";
  ...
  desired.push(
    ...
    {
      path: portablePath(
        join(".agents", "skills", "omc-orchestration", "SKILL.md"),
      ),
      kind: "file",
      content: skill,
    },
  ...
  for (const entry of readdirSync(join(packageRoot, "agents"), {
    ...
    desired.push({
      path: portablePath(
        join(".codex", "agents", `${entry.name.slice(0, -3)}.toml`),
      ),
      kind: "file",
...
function prepareSetupMutations(
  root: string,
  receipt: ProjectHostReceipt,
  hosts: readonly OrchestratorHost[],
  packageRoot: string,
  guidance: string,
): {
  mutations: Mutation[];
  nextReceipt: ProjectHostReceipt;
  receiptAssets: Record<OrchestratorHost, DesiredAsset[]>;
} {
  const receiptAssets = {} as Record<OrchestratorHost, DesiredAsset[]>;
  const nextHosts: Partial<Record<OrchestratorHost, HostReceipt>> = {
    ...receipt.hosts,
  };
  ...
  for (const host of hosts) {
    ...
    nextHosts[host] = {
      ...
      assets: desired.map((asset) => ({
        ...
        kind: asset.kind,
        digest: sha256(
          asset.kind === "file" ? asset.content : (asset.managedText ?? ""),
        ),
  ...
  for (const [relativePath, asset] of desiredByPath) {
    ...
    const previousOwner = hosts
      .map((host) =>
        receipt.hosts[host]?.assets.find((item) => item.path === relativePath),
      )
  ...
  for (const host of hosts) {
    const wanted = new Set(
      (receiptAssets[host] ?? []).map((asset) => portablePath(asset.path)),
    );
    for (const old of receipt.hosts[host]?.assets ?? []) {
      if (wanted.has(old.path) || desiredByPath.has(old.path)) continue;
  ...
  const nextReceipt: ProjectHostReceipt = {
    schemaVersion: 1,
    hosts: nextHosts,
...
function mergedConfig(
  ...
  requested: readonly OrchestratorHost[],
): OrchestratorRepositoryConfig {
  const supportedHosts = HOST_ORDER.filter(
    (host) =>
      current?.supportedHosts.includes(host) || requested.includes(host),
  );
  const defaultHost =
    current?.defaultHost ??
...
export async function setupProjectHosts(
  cwd: string,
  hostsInput: readonly OrchestratorHost[],
  options: SetupProjectHostsOptions,
): Promise<SetupProjectHostsResult> {
  ...
  const guidance = readSource(
    join(packageRoot, "templates", "hosts", "orchestrator-guidance.md"),
  );
  readSource(join(packageRoot, "bridge", "mcp-server.cjs"));
  const receipt = parseReceipt(root);

  if (options.dryRun) {
    const plan = prepareSetupMutations(
      root,
      receipt,
      hosts,
      packageRoot,
  ...
  let transaction: AppliedTransaction | undefined;
  try {
...
function uninstallMutations(
  ...
  if (!owned) throw new Error(`host_assets_not_installed: ${host}`);
  const mutations: Mutation[] = [];
...
export async function uninstallProjectHost(
  cwd: string,
  host: OrchestratorHost,
): Promise<UninstallProjectHostResult> {
  if (!HOST_ORDER.includes(host)) throw new Error("orchestrator_invalid_host");
  const root = resolveOrchestratorPaths(cwd).repositoryRoot;
  let transaction: AppliedTransaction | undefined;
  let preserved: string[] = [];
  let remainingHosts: OrchestratorHost[] = [];
  try {
    await updateOrchestratorRepositoryConfig(root, (current) => {
      if (!current || !current.supportedHosts.includes(host)) {
        throw new Error(`orchestrator_host_not_supported: ${host}`);
      }
      remainingHosts = current.supportedHosts.filter(
      ...
      const defaultHost =
        current.defaultHost === host ? remainingHosts[0] : current.defaultHost;
      return {
        schemaVersion: 1,
  ...
  return {
    repositoryRoot: root,
    host,
    removedFiles: (transaction?.changedFiles ?? []).filter(
      (path) => path !== portablePath(RECEIPT_PATH),
    ),
...
function codexNativeHooksSupported(cli: CliProbeResult): boolean {
...
export async function doctorProjectHosts(
  cwd: string,
): Promise<ProjectHostsDoctorResult> {
  const root = resolveOrchestratorPaths(cwd).repositoryRoot;
  const receipt = parseReceipt(root);
  ...
  if (!config)
    generalIssues.push(
      "Repository host setup is not configured. Run omc setup --host both --scope project.",
    );
  generalIssues.push(...projectIgnoreIssues(root, receipt));

  const entries = {} as Record<OrchestratorHost, HostAssetDoctorEntry>;
  for (const host of HOST_ORDER) {
    ...
    if (installed && !config?.supportedHosts.includes(host))
      issues.push(
        "Host assets exist but the shared config does not support this host.",
      );
    ...
    if (config?.supportedHosts.includes(host) && !cli.found)
      issues.push(
        `CLI unavailable: ${cli.error ?? `${host} --version failed`}`,
      );
    const nativeHooksSupported =
      host === "claude" ? cli.found : codexNativeHooksSupported(cli);
    const hookDiagnostics = readNativeHookDiagnostics(
      root,
      host,
      !cli.found
        ? "cli-unavailable"
  ...
  const configuredHealthy =
    config?.supportedHosts.every((host) => entries[host].healthy) ?? false;
