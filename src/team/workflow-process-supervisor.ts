...
export function buildWindowsWorkflowSupervisorSource(): string {
  ...
  return [
    ...
    interop,
    '"@',
    '$pi = New-Object O+PROCESS_INFORMATION; $job = [IntPtr]::Zero; $envPtr = [IntPtr]::Zero; $attributeList = [IntPtr]::Zero; $handleList = [IntPtr]::Zero; $stdin = [IntPtr]::Zero; $stdout = [IntPtr]::Zero; $stderr = [IntPtr]::Zero; $exitCode = [uint32]1',
    'try {',
    ...
    '  $attributeBytes = [UIntPtr]::Zero; [O]::InitializeProcThreadAttributeList([IntPtr]::Zero, 1, 0, [ref]$attributeBytes) | Out-Null; $attributeList = [Runtime.InteropServices.Marshal]::AllocHGlobal([int64]$attributeBytes.ToUInt64()); if (-not [O]::InitializeProcThreadAttributeList($attributeList, 1, 0, [ref]$attributeBytes)) { throw "workflow_supervisor_attribute_init_failed" }; $handleList = [Runtime.InteropServices.Marshal]::AllocHGlobal([IntPtr]::Size * 3); [Runtime.InteropServices.Marshal]::WriteIntPtr($handleList, 0, $stdin); [Runtime.InteropServices.Marshal]::WriteIntPtr($handleList, [IntPtr]::Size, $stdout); [Runtime.InteropServices.Marshal]::WriteIntPtr($handleList, [IntPtr]::Size * 2, $stderr); $handleBytes = [UIntPtr]([uint64]([IntPtr]::Size * 3)); if (-not [O]::UpdateProcThreadAttribute($attributeList, 0, [IntPtr]0x00020002, $handleList, $handleBytes, [IntPtr]::Zero, [IntPtr]::Zero)) { throw "workflow_supervisor_attribute_update_failed" }',
    '  $intendedNames = @($payload.environment_keys); $repairs = @($payload.environment_repairs); $repairValues = @{}; if ($repairs.Count -gt 1) { throw "workflow_supervisor_environment_repair_invalid" }; foreach ($repair in $repairs) { if ([string]$repair.name -ine "PSMODULEPATH" -or $intendedNames -notcontains [string]$repair.name) { throw "workflow_supervisor_environment_repair_invalid" }; $repairValues[[string]$repair.name] = [string]$repair.value }',
    ...
    '  $envText = (($envPairs -join [char]0) + [char]0 + [char]0); $envBytes = [Text.Encoding]::Unicode.GetBytes($envText); $envPtr = [Runtime.InteropServices.Marshal]::AllocHGlobal($envBytes.Length); [Runtime.InteropServices.Marshal]::Copy($envBytes, 0, $envPtr, $envBytes.Length)',
    '  $argv = [string[]]$payload.argv; $cmd = [O]::BuildCommandLine($argv, [bool]$payload.windows_verbatim_arguments); $application = if ([IO.Path]::IsPathRooted($argv[0])) { $argv[0] } else { $null }; $si = New-Object O+STARTUPINFOEX; $startup = New-Object O+STARTUPINFO; $startup.cb = [Runtime.InteropServices.Marshal]::SizeOf($si); $startup.dwFlags = 0x100; $startup.hStdInput = $stdin; $startup.hStdOutput = $stdout; $startup.hStdError = $stderr; $si.StartupInfo = $startup; $si.lpAttributeList = $attributeList; $flags = 0x00000004 -bor 0x00000400 -bor 0x00080000 -bor 0x08000000; if (-not [O]::CreateProcessW($application, $cmd, [IntPtr]::Zero, [IntPtr]::Zero, $true, $flags, $envPtr, [string]$payload.cwd, [ref]$si, [ref]$pi)) { throw ("workflow_supervisor_create_process_failed:" + [Runtime.InteropServices.Marshal]::GetLastWin32Error()) }',
    '  $job = [O]::CreateJobObjectW([IntPtr]::Zero, $null); if ($job -eq [IntPtr]::Zero) { throw "workflow_supervisor_create_job_failed" }; $info = New-Object O+JOBOBJECT_EXTENDED_LIMIT_INFORMATION; $basic = New-Object O+JOBOBJECT_BASIC_LIMIT_INFORMATION; $basic.LimitFlags = 0x2000; $info.BasicLimitInformation = $basic; $infoPtr = [Runtime.InteropServices.Marshal]::AllocHGlobal([Runtime.InteropServices.Marshal]::SizeOf($info)); try { [Runtime.InteropServices.Marshal]::StructureToPtr($info, $infoPtr, $false); if (-not [O]::SetInformationJobObject($job, 9, $infoPtr, [Runtime.InteropServices.Marshal]::SizeOf($info))) { throw "workflow_supervisor_job_config_failed" } } finally { [Runtime.InteropServices.Marshal]::FreeHGlobal($infoPtr) }; if (-not [O]::AssignProcessToJobObject($job, $pi.hProcess)) { throw "workflow_supervisor_assign_job_failed" }; if ([O]::ResumeThread($pi.hThread) -eq [uint32]::MaxValue) { throw "workflow_supervisor_resume_failed" }',
    ...
    '} catch { if ($pi.hProcess -ne [IntPtr]::Zero) { if ($job -ne [IntPtr]::Zero) { [O]::TerminateJobObject($job, 1) | Out-Null }; [O]::TerminateProcess($pi.hProcess, 1) | Out-Null; [O]::WaitForSingleObject($pi.hProcess, 5000) | Out-Null }; [Console]::Error.WriteLine($_.Exception.Message); $exitCode = 1 } finally { if ($attributeList -ne [IntPtr]::Zero) { [O]::DeleteProcThreadAttributeList($attributeList); [Runtime.InteropServices.Marshal]::FreeHGlobal($attributeList) }; if ($handleList -ne [IntPtr]::Zero) { [Runtime.InteropServices.Marshal]::FreeHGlobal($handleList) }; if ($envPtr -ne [IntPtr]::Zero) { [Runtime.InteropServices.Marshal]::FreeHGlobal($envPtr) }; foreach ($handle in @($stdin, $stdout, $stderr, $pi.hThread, $pi.hProcess, $job)) { if ($handle -ne [IntPtr]::Zero) { [O]::CloseHandle($handle) | Out-Null } } }',
...
export function superviseWindowsWorkflowInvocation(input: {
  ...
  const environmentRepairs = intendedEnvironment
    .filter(entry => entry.canonicalName === 'PSMODULEPATH')
  ...
  // Do not let the wrapper discover modules through a caller-controlled path.
  // The intended value is carried as the sole repair and restored only for the provider block.
