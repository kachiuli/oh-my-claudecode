...
export interface WorkerStatus {
  workerName: string;
  provider: 'claude' | 'codex' | 'gemini' | 'grok' | 'cursor' | 'antigravity' | 'glm' | 'mimo';
...
export function getTeamStatus(
  ...
  const workers: WorkerStatus[] = mcpWorkers.map(w => {
    ...
    const provider = w.agentType.replace(/^(?:mcp|tmux)-/, '') as 'claude' | 'codex' | 'gemini' | 'grok' | 'cursor' | 'antigravity' | 'glm' | 'mimo';
