import { parseWorkflowState, type VersionedWorkflowState } from './workflow-contracts.js';
...
function summarize(attempts: WorkflowTelemetry[], totalInvocations = attempts.length) {
  return {
...
export function workflowUsage(input: VersionedWorkflowState) {
  ...
  return {
    ...
    ...(state.schemaVersion === 2 && bound ? {
      schemaVersion: 2 as const, profile: state.profile,
      ...
      reviewAttemptRelations: Object.fromEntries((['independent', 'self-review', 'unknown'] as const)
