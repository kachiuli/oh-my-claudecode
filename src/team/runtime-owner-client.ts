...
import { isValidTeamInstanceId, type RecoverDeadWorkerV2Failure, type RecoverDeadWorkerV2Result, type TeamInstanceId } from './types.js';
...
function normalizeRecoveryInstanceId(value: unknown): TeamInstanceId {
  if (!isValidTeamInstanceId(value)) throw new Error('invalid_recovery_instance_id');
  return value.toLowerCase();
}

function recoveryInstanceBinding(input: Pick<RecoverDeadWorkerOwnerInput, 'teamName' | 'cwd' | 'instanceId'>) {
...
async function assertRecoveryInstanceUnderLock(
  input: Pick<RecoverDeadWorkerOwnerInput, 'teamName' | 'cwd' | 'instanceId'>,
): Promise<void> {
  await assertTeamInstanceUnderLock(recoveryInstanceBinding(input));
}

async function assertRecoveryInstance(
  input: Pick<RecoverDeadWorkerOwnerInput, 'teamName' | 'cwd' | 'instanceId'>,
): Promise<boolean> {
  try {
  ...
  try {
    for (const name of readdirSync(absPath(cwd, TeamPaths.recoveryRequestsRoot()))) {
      ...
      if (!reservation || reservation.operation !== payload.operation || reservation.payload_hash !== targetHash
        ...
        || reservation.instance_id.toLowerCase() !== payload.instanceId.toLowerCase()) continue;
...
export async function teamRecoveryState(cwd: string, teamName: string): Promise<TeamRecoveryState> {
...
async function bootstrapPersistentOwner(input: RecoverDeadWorkerOwnerInput, priorEpoch: number | null): Promise<boolean> {
  type BootstrapSetup = {
    ...
    child: ChildProcess;
    childPid: number;
    childProcessStartedAt: string;
  };
  let childFailed = false;
  const onChildFailure = () => { childFailed = true; };
  let setup: BootstrapSetup | null = null;
  try {
    ...
    setup = await withTeamInstanceLifecycleLock(input.cwd, input.teamName, async () => {
      let predecessor;
      try { predecessor = readLatestOwnerEpoch(input.cwd, input.teamName); } catch { return null; }
      ...
      if (priorEpoch !== (predecessor?.epoch ?? null)
        ...
        || reservation.instance_id.toLowerCase() !== normalizeRecoveryInstanceId(input.instanceId)) return null;
      try {
      ...
      if (hasLiveOrUnknownBootstrapCandidate(input, reservation.recovery_id, expectedEpoch, predecessor)) return null;
      ...
      if (typeof childPid !== 'number' || !Number.isSafeInteger(childPid) || childPid < 1) return null;
      const childProcessStartedAt = currentProcessStartIdentity(childPid);
      if (!childProcessStartedAt || childFailed) return null;
      try {
        await publishRecoveryOwnerBootstrapCandidate(input, reservation.recovery_id, expectedEpoch, bootstrapNonce,
          childPid, childProcessStartedAt, predecessor);
      } catch {
        return null;
      }
  ...
  return {
    async recoverDeadWorker(input: RecoverDeadWorkerOwnerInput): Promise<RecoverDeadWorkerV2Result> {
      ...
      const normalized = { ...input, requestId, instanceId: normalizeRecoveryInstanceId(input.instanceId),
        timeoutMs: timeoutBudget(input.timeoutMs,
        timing.minTimeoutMs ?? MIN_RECOVERY_TIMEOUT_MS, timing.maxTimeoutMs ?? MAX_RECOVERY_TIMEOUT_MS) };
      const payload: RecoveryRequestPayload = { operation: 'recover-worker', workspaceHash: workspaceHash(normalized.cwd),
      ...
      const admitted = await withTeamInstanceLifecycleLock(
        normalized.cwd, normalized.teamName,
        async () => {
          ...
          if (existingMatchesPayload) {
            ...
            if (isMatchingRecoveryFinal(existingOutcome, {
              ...
              return { kind: 'final' as const, result: existingOutcome.result };
            }
          }
          ...
          if (!existing || existingMatchesPayload || active) {
            try {
              ...
              return { kind: 'blocked' as const, recoveryId: existing?.recovery_id ?? '' };
            }
          }
      ...
      if (timing.persistentOwnerBootstrap) {
        const state = await teamRecoveryState(normalized.cwd, normalized.teamName);
        let ownerReady: boolean | null = false;
        if (state === 'v2') {
          ...
          if (availability === 'unknown') ownerReady = null;
            ...
            try { priorEpoch = readLatestOwnerEpoch(normalized.cwd, normalized.teamName)?.epoch ?? null; } catch { priorEpoch = null; }
