...
import {
  ...
  type RecoverDeadWorkerV2Result,
  type TeamInstanceId,
...
function normalizeRuntimeInstanceId(value: unknown): TeamInstanceId | null {
  return isValidTeamInstanceId(value) ? value.toLowerCase() : null;
}

function runtimeInstanceBinding(teamName: string, cwd: string, instanceId: string): ReturnType<typeof createTeamInstanceBinding> {
  const normalized = normalizeRuntimeInstanceId(instanceId);
  if (!normalized) throw new Error('team_instance_identity_missing');
...
async function assertRuntimeInstance(
  teamName: string,
  cwd: string,
  ...
  try {
    await withTeamInstanceLifecycleLock(binding.cwd, binding.team_name, () =>
      assertTeamInstanceUnderLock(binding),
    );
    return true;
  } catch {
    return false;
...
export async function handleRecoverDeadWorkerV2Owner(
  ...
  if (intent.request_id !== input.requestId || intent.worker_name !== input.workerName
    ...
    || intent.instance_id.toLowerCase() !== normalizedInstanceId) throw new Error('invalid_persisted_state');
...
export async function processPendingRecoveryIntents(
  ...
  if (!normalizedExpectedInstanceId) return;
  ...
  for (const name of names) {
    ...
    try {
      ...
      const ownerInput: RecoverDeadWorkerOwnerInput = {
        teamName,
        cwd,
...
async function updateAllDeadRecoveryGraceUnderLock(
  teamName: string,
  cwd: string,
...
export async function updateAllDeadRecoveryGrace(
  ...
  const normalized = normalizeRuntimeInstanceId(instanceId);
  if (!normalized) return { deadlineAt: null, expired: false };
  const binding = runtimeInstanceBinding(teamName, cwd, normalized);
  return withTeamInstanceLifecycleLock(binding.cwd, binding.team_name, async () => {
    await assertTeamInstanceUnderLock(binding);
...
export function hasPendingRecoveryAdmissionBeforeDeadline(
  ...
  for (const name of names) {
    ...
    try {
      ...
      if (reservation.instance_id.toLowerCase() !== normalizedExpectedInstanceId) continue;
...
export async function fenceAllDeadRecoveryExpiry(teamName: string, cwd: string, deadlineAt: number, instanceId: string): Promise<boolean> {
  const normalized = normalizeRuntimeInstanceId(instanceId);
  if (!normalized) return false;
  const binding = runtimeInstanceBinding(teamName, cwd, normalized);
  return withTeamInstanceLifecycleLock(binding.cwd, binding.team_name, async () => {
    ...
    if (hasPendingRecoveryAdmissionBeforeDeadline(teamName, cwd, deadlineAt, normalized)
      || hasPendingRecoveryIntentBeforeDeadline(teamName, cwd, deadlineAt, normalized)) return false;
    const nextRevision = current.stateRevision + 1;
    ...
    } as NonNullable<PersistedTeamConfig['shutdown_attempt']> & { instance_id?: string };
...
export interface PersistentRecoveryOwnerLoopOptions {
  ...
  shutdown?: (teamName: string, cwd: string, options: { force: boolean; instanceId: string }) => Promise<void>;
}
...
export async function runPersistentRecoveryOwnerLoop(
  ...
  const processIntents = options.processIntents
    ?? ((teamName: string, cwd: string, instanceId: string) =>
      processPendingRecoveryIntents(teamName, cwd, undefined, instanceId));
  ...
  while (options.shouldContinue?.(iteration) ?? true) {
    ...
    if (!current || !current.config.instance_id
      ...
      || current.config.lifecycle_state === 'stopped') return;
    ...
    if (bootstrapPending) {
      ...
      try {
        if (!await assertRuntimeInstance(input.teamName, input.cwd, expectedInstanceId)) return;
        await execute(input);
    ...
    if (!afterIntents || !afterIntents.config.instance_id
      ...
      || afterIntents.config.lifecycle_state === 'stopped') return;
...
export function buildCliOutput(
  ...
  if (!normalizedInstanceId) throw new Error('result_instance_identity_missing');
 ...
 */
export async function captureTerminalCliResult(
  cwd: string,
  stateRoot: string,
  teamName: string,
  phase: TerminalPhaseResult,
...
async function writePanesFile(
  ...
  if (!jobId || !omcJobsDir) return;
  const normalized = normalizeRuntimeInstanceId(instanceId);
  if (!normalized) throw new Error('pane_artifact_instance_mismatch');
  const binding = runtimeInstanceBinding(teamName, cwd, normalized);
  await withTeamInstanceLifecycleLock(binding.cwd, binding.team_name, async () => {
    await assertTeamInstanceUnderLock(binding);
    const current = await readRevisionedTeamConfig(teamName, cwd);
...
export async function finalizeRuntimeShutdown<T>(
  ...
  instance: { teamName: string; cwd: string; instanceId: string },
): Promise<T> {
...
async function main(): Promise<void> {
  ...
  if (typeof input.cwd !== 'string' || input.cwd.trim().length === 0) missing.push('cwd');
  if (!input.instanceId) missing.push('instanceId');
  if (missing.length > 0) {
  ...
  if (!isRuntimeV2Enabled()) {
    process.stderr.write(
      '[runtime-cli] team_start_unsafe_runtime_v1: instance-bound provider cleanup requires runtime v2; set OMC_RUNTIME_V2=1\n',
    );
    process.exit(1);
    return;
  }
  ...
  async function performShutdown(status: 'completed' | 'failed'): Promise<void> {
    ...
    const output = await finalizeRuntimeShutdown(
      async () => buildCliOutput(stateRoot, teamName, finalStatus, workerCount, startTime, instanceId),
      async () => {
        if (!runtime) return;
        try {
  ...
  async function exitWithoutShutdown(phase: TerminalPhaseResult): Promise<void> {
    pollActive = false;
...
function decodeCanonicalBase64UrlJson(value: string): unknown {
  if (!/^[A-Za-z0-9_-]+$/.test(value)) throw new Error('tmux_server_identity_guard_encoding_invalid');
  let decoded: string;
  try {
...
export function parseTmuxServerIdentityGuardArgs(
  argv: readonly string[] = process.argv,
): TmuxServerIdentityGuardInput {
  const flag = '--tmux-server-identity-guard';
  const indexes = argv
    .map((arg, index) => arg === flag ? index : -1)
    .filter(index => index >= 0);
  if (indexes.length !== 1) throw new Error('tmux_server_identity_guard_argument_conflict');
...
export function runTmuxServerIdentityGuardFromArgs(
  argv: readonly string[] = process.argv,
): 0 | 1 {
