...
const mockedCalls = vi.hoisted(() => ({
  ...
  newWindowStdouts: [] as string[],
  newWindowError: null as { stdout: string; stderr: string; message: string } | null,
  freshServerAlive: false,
  freshServerKillCount: 0,
  freshSessionInventory: [] as string[],
  nativeWindowInventory: [] as string[],
...
vi.mock('child_process', async (importOriginal) => {
  ...
  const runMockExec = (args: string[]): { stdout: string; stderr: string } => {
    ...
    if (tmuxArgs[0] === 'display-message' && tmuxArgs.some(arg => arg.includes('#{socket_path}'))) {
      if (!socket && !process.env.TMUX) return { stdout: '', stderr: '' };
...
function destructiveProviderCalls(): string[][] {
  return mockedCalls.execFileArgs.filter((args) =>
    /\b(?:kill-server|kill-session|kill-window|kill-pane|close-surface)\b/.test(args.join(' ')),
  );
}
...
describe('createTeamSession context resolution', () => {
  ...
  it.skipIf(!supportsStrictTmuxFixture)('creates a detached session when running outside tmux', async () => {
    ...
    const keepaliveCall = mockedCalls.execFileArgs.find((args) =>
      args.includes('start-server') && args.includes('exit-empty'),
    );
    ...
    const detachedCreateCall = mockedCalls.execFileArgs.find((args) =>
      args.join(' ').includes('new-session') && args.includes('if-shell'),
    );
    ...
    expect(mockedCalls.execFileArgs.some((args) =>
      args.includes('if-shell') && args.join(' ').includes('set-clipboard'),
    )).toBe(true);
    expect(mockedCalls.execFileArgs.some((args) =>
      args.includes('if-shell') && /exit-empty(?:['"\\]|\s)+on/.test(args.join(' ')),
    )).toBe(true);
    expect(mockedCalls.execFileArgs.some((args) =>
      args.includes('if-shell')
      && args.join(' ').includes(`=${session.sessionName.split(':')[0]}:`)
      && args.join(' ').includes('set-option'),
    )).toBe(true);
    expect(mockedCalls.execFileArgs.some((args) =>
      args.includes('if-shell') && args.join(' ').includes('terminal-features'),
    )).toBe(false);
  ...
  it.skipIf(!supportsStrictTmuxFixture)('anchors context to TMUX_PANE to avoid focus races', async () => {
    ...
    expect(mockedCalls.execFileArgs.some((args) =>
      args.includes('-S') && args.includes(strictSocketPath) && args.join(' ').includes('set-clipboard'),
    )).toBe(true);
    expect(mockedCalls.execFileArgs.some((args) =>
      args.includes('if-shell')
  ...
  it.skipIf(!supportsStrictTmuxFixture).each([
  ...
  ] as const)('keeps shared-tmux malformed split cleanup unknown with %s', async (
    _label,
    splitStdouts,
    expectedKnownPanes,
  ) => {
    ...
    const creationError = caught as TeamSessionCreationError;
  ...
  it('rejects native Windows psmux detached team sessions without strict tmux authority', async () => {
    ...
    expect(mockedCalls.execFileArgs.some((args) =>
      /\b(?:start-server|new-session|new-window|split-window)\b/.test(args.join(' ')),
    )).toBe(false);
  });
...
describe('splitTeamWorkerPane multiplexer routing (#3267)', () => {
  ...
  it.skipIf(!supportsStrictTmuxFixture)('uses an original-server identity for tmux split effects', async () => {
    ...
    const evidence = await splitTeamWorkerPaneWithEvidence(
      '%732',
      'down',
      '/tmp',
      'tmux',
