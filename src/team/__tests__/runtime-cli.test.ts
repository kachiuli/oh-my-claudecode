...
describe('runtime-cli v2 shutdown publication', () => {
  it('captures an instance-bound snapshot before shutdown and publishes it afterward', async () => {
    ...
    try {
      ...
      const phases: string[] = [];
      let published: { instanceId?: string; taskResults: Array<{ taskId: string; status: string; summary: string }> } | undefined;
  ...
  it('captures terminal output only under the reserved instance authority', async () => {
    ...
    try {
      ...
      const result = await captureTerminalCliResult(
        cwd,
        stateRoot,
        teamName,
        'complete',
...
describe('runtime-cli result artifact writer', () => {
  ...
  it('no-ops when jobs dir is missing even if job id is provided', async () => {
    ...
    try {
      ...
      if (previousJobsDir === undefined) delete process.env.OMC_JOBS_DIR;
      else process.env.OMC_JOBS_DIR = previousJobsDir;
      rmSync(jobsDir, { recursive: true, force: true });
    }
  });
  ...
  it('rejects result publication without a valid instance identity', async () => {
    ...
    try {
      await expect(writeResultArtifact(
        ...
        'omc-invalid',
        jobsDir,
      )).rejects.toThrow('result_artifact_instance_identity_missing');
      expect(readdirSync(jobsDir)).toEqual([]);
    } finally {
      rmSync(jobsDir, { recursive: true, force: true });
    }
...
describe('runtime-cli recovery pane refresh', () => {
  ...
  it('rejects authoritative pane refresh from a replacement instance', async () => {
    ...
    try {
      ...
      await expect(refreshRuntimeWorkerPaneIds(
        { workerPaneIds: [] },
        teamName,
        cwd,
        INSTANCE_ID,
      )).rejects.toThrow('team_instance_identity_mismatch');
    } finally {
      rmSync(cwd, { recursive: true, force: true });
    }
  });
...
describe('runtime-cli recovery intent cleanup', () => {
  ...
  async function seedIntent(cwd: string, requestId: string, recoveryId: string): Promise<string> {
  ...
  async function seedExpiredAllDeadGrace(
    cwd: string,
    deadline: number,
  ...
  function seedPendingAdmission(
    ...
    const path = absPath(cwd, TeamPaths.recoveryRequestPending(requestId));
    const bytes = readFileSync(path, 'utf8').replace(/"created_at":"[^"]+"/,
      `"created_at":"${new Date(deadline - 1_000).toISOString()}"`);
  ...
  it('expires instance B while retaining a valid predeadline admission owned by instance A', async () => {
    ...
    try {
      ...
      const { path, bytes } = seedPendingAdmission(
        cwd,
        'request-foreign-instance',
        'recovery-foreign-instance',
        'intent-team',
        INSTANCE_ID,
        deadline,
      );
  ...
  it('keeps expiry fenced by a valid predeadline admission owned by the current instance', async () => {
    ...
    try {
      ...
      const { path, bytes } = seedPendingAdmission(
        cwd,
        'request-matching-instance',
        'recovery-matching-instance',
        'intent-team',
        OTHER_INSTANCE_ID,
        deadline,
      );
  ...
  it('keeps expiry fenced by unidentifiable predeadline admission evidence', async () => {
    ...
    try {
      const deadline = Date.now() - 1_000;
      const configPath = await seedExpiredAllDeadGrace(cwd, deadline, OTHER_INSTANCE_ID);
      const { path } = seedPendingAdmission(
        cwd,
        'request-corrupt-admission',
        'recovery-corrupt-admission',
        'intent-team',
        INSTANCE_ID,
        deadline,
      );
