...
import type { TeamConfig, TmuxServerIdentity } from '../../team/types.js';
...
const tmuxMocks = vi.hoisted(() => ({
  ...
  captureOwnedTeamPane: vi.fn<typeof import('../../team/tmux-session.js').captureOwnedTeamPane>(async () => ''),
...
const serverMocks = vi.hoisted(() => ({
  handlers: [] as Array<(request: unknown) => Promise<unknown>>,
}));
const runtimeMocks = vi.hoisted(() => ({
  isRuntimeV2Enabled: vi.fn(() => true),
  shutdownTeamV2: vi.fn(async (..._args: unknown[]): Promise<
    | { outcome: 'cleaned' }
    | { outcome: 'preserved'; reason: string; workers: string[] }
    | { outcome: 'failed'; reason: string; detail: string }
  > => ({ outcome: 'cleaned' })),
  actualShutdownTeamV2: undefined as undefined | ((teamName: string, cwd: string, options?: Record<string, unknown>) => Promise<unknown>),
  resumeTeam: vi.fn(),
  shutdownTeam: vi.fn(),
}));
const monitorMocks = vi.hoisted(() => ({
  readTeamConfig: vi.fn(),
  actualReadTeamConfig: undefined as undefined | ((teamName: string, cwd: string) => Promise<TeamConfig | null>),
}));
...
vi.mock('@modelcontextprotocol/sdk/server/index.js', async (importOriginal) => {
  ...
  class CapturingServer {
    setRequestHandler(_schema: unknown, handler: (request: unknown) => Promise<unknown>): void {
...
vi.mock('../../team/runtime-v2.js', async (importOriginal) => {
  ...
  runtimeMocks.actualShutdownTeamV2 = actual.shutdownTeamV2 as typeof runtimeMocks.actualShutdownTeamV2;
  return {
    ...actual,
...
const capturedTmuxProcessIdentity = (process.platform === 'darwin' || process.platform === 'linux')
  ...
  : null;
const capturedTmuxServerIdentity: TmuxServerIdentity | undefined = capturedTmuxProcessIdentity
  ? {
...
function paneArtifact(
  instanceId: string,
  paneIds: string[],
  leaderPaneId = '%1',
  sessionName = 'team-one:0',
...
function jobRecord(teamName: string, cwd: string, instanceId = INSTANCE_ID): Record<string, unknown> {
...
function boundedSignal(signal: Promise<void>, timeoutMs: number): Promise<boolean> {
...
const NUDGE_SERVER_IDENTITY: TmuxServerIdentity = {
  socket_path: '/tmp/omc-idle-nudge-custom.sock',
  server_pid: process.pid,
  process_started_at: capturedTmuxProcessIdentity ?? 'test-process',
};

interface NudgeTeamFixture {
  binding: ReturnType<typeof createTeamInstanceBinding>;
  config: TeamConfig;
  paneId: string;
  leaderPaneId: string;
  sessionName: string;
...
function buildNudgeConfig(
  binding: NudgeTeamFixture['binding'],
  paneId: string,
  leaderPaneId: string,
  sessionName: string,
  serverIdentity?: TmuxServerIdentity,
...
async function createActiveNudgeTeam(
  teamName: string,
  cwd: string,
  instanceId: string,
  provider: 'tmux' | 'cmux' = 'tmux',
  serverIdentity: TmuxServerIdentity | undefined = NUDGE_SERVER_IDENTITY,
): Promise<NudgeTeamFixture> {
...
async function createReplacementNudgeFixture(
  fixture: NudgeTeamFixture,
  instanceId: string,
): Promise<NudgeTeamFixture> {
  ...
  await withTeamInstanceLifecycleLock(binding.cwd, binding.team_name, async () => {
    ...
    rmSync(
      absPath(
        fixture.binding.cwd,
        TeamPaths.teamInstanceReservation(fixture.binding.workspace_hash, fixture.binding.team_name),
      ),
      { force: true },
    );
...
function callCapturedTool(arguments_: Record<string, unknown>, name: string): Promise<unknown> {
  const handler = serverMocks.handlers.at(-1);
  if (!handler) throw new Error('team server call handler was not registered');
...
describe('team-server artifact convergence + scoped cleanup', () => {
  ...
  it('handleStatus converges to terminal artifact before pid liveness', async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({ ...jobRecord('artifact-team', '/tmp/artifact-team'), pid: 999999 }),
      'utf-8',
    );

    writeFileSync(
      join(jobsDir, `${jobId}-result.json`),
      JSON.stringify({ status: 'completed', instanceId: INSTANCE_ID, teamName: 'artifact-team', taskResults: [] }),
      'utf-8',
    );
  ...
  it('handleWait deterministically fails on parse-failed artifact and persists failure', async () => {
    ...
    writeFileSync(
      join(jobsDir, `${jobId}.json`),
      JSON.stringify({ ...jobRecord('artifact-team', '/tmp/artifact-team'), pid: process.pid }),
      'utf-8',
    );
  ...
  it('handleWait does not send to a replacement after the original pane capture loses authority', async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify(paneArtifact(INSTANCE_ID, [fixture.paneId], fixture.leaderPaneId, fixture.sessionName)),
      'utf-8',
    );
    ...
    const captureGate = new Promise<void>(resolve => { captureStarted = resolve; });
    tmuxMocks.captureOwnedTeamPane.mockImplementationOnce(async ownership => {
      ...
      captureStarted();
      return new Promise<string>(resolve => { releaseCapture = resolve; });
    });
    ...
    const replacementConfig = JSON.parse(readFileSync(
      join(createTeamInstanceBinding({
        teamName,
        cwd,
        instanceId: OTHER_INSTANCE_ID,
    ...
    const replacementReservation = JSON.parse(readFileSync(
      absPath(
        cwd,
        TeamPaths.teamInstanceReservation(replacementBinding.workspace_hash, teamName),
      ),
      'utf-8',
    )) as Record<string, unknown>;
  ...
  it('handleWait retains the original lifecycle lock across deferred tmux input', async () => {
    ...
    const sendGate = new Promise<void>(resolve => { sendStarted = resolve; });
    tmuxMocks.sendToWorker.mockImplementationOnce(async (...args: unknown[]) => {
      expect(args).toEqual([
        fixture.sessionName,
        fixture.paneId,
        expect.any(String),
    ...
    let competingSetupError: unknown;
    try {
      if (sendObserved) {
    ...
    if (competingSetupError) throw competingSetupError;
    ...
    expect(tmuxMocks.sendToWorker).toHaveBeenCalledWith(
      fixture.sessionName,
      fixture.paneId,
      expect.any(String),
      NUDGE_SERVER_IDENTITY,
  ...
  it('handleWait rechecks the durable job under its lock immediately before input', async () => {
    ...
    monitorMocks.readTeamConfig.mockImplementation(async (readTeamName: string, readCwd: string) => {
  ...
  it('does not let child close overwrite a terminal artifact converged first', async () => {
    ...
    }, 'omc_run_team_start') as { content: Array<{ text: string }> };
    ...
    const before = JSON.parse(readFileSync(join(jobsDir, `${jobId}.json`), 'utf-8')) as Record<string, unknown>;
    ...
    const after = JSON.parse(readFileSync(join(jobsDir, `${jobId}.json`), 'utf-8')) as Record<string, unknown>;
  ...
  it('marks a code-zero child with empty stdout failed without terminal proof', async () => {
    ...
    const saved = JSON.parse(readFileSync(join(jobsDir, `${jobId}.json`), 'utf-8')) as Record<string, unknown>;
  ...
  it.each(['replacement', 'removal'] as const)(
    'does not overwrite a durable job %s that changes before synchronous MCP spawn throws',
    async mode => {
      await importTeamServerWithJobsDir(jobsDir);
      let spawnJobsDir: string | undefined;
      childMocks.spawn.mockImplementationOnce((
        ...
        if (!jobId) throw new Error('spawn job identity is missing');
      ...
      const response = await callCapturedTool({
        ...
        tasks: [{ subject: 'one', description: 'desc' }],
        cwd: join(testRoot, 'sync-mcp-spawn-workspace'),
      }, 'omc_run_team_start') as { content: Array<{ text: string }>; isError?: boolean };
      ...
      if (mode === 'replacement') {
        ...
        const saved = JSON.parse(readFileSync(join(durableJobsDir, files[0]), 'utf-8')) as Record<string, unknown>;
  ...
  it('merges current status and result fields after deferred cleanup succeeds', async () => {
    ...
    let release!: (value: { outcome: 'cleaned' }) => void;
    ...
    runtimeMocks.shutdownTeamV2.mockImplementationOnce(async () => {
      resolveCalled();
      return new Promise<{ outcome: 'cleaned' }>(resolve => { release = resolve; });
    });

    const cleanupPromise = handleCleanup({ job_id: jobId, grace_ms: 0 });
    const cleanupSettled = cleanupPromise.then(
      value => ({ kind: 'resolved' as const, value }),
      error => ({ kind: 'rejected' as const, error }),
    );
    const entered = await Promise.race([
      called.then(() => 'entered' as const),
      cleanupSettled.then(() => 'settled' as const),
    ]);
  ...
  it('does not regress a newer cleanup blocked reason during deferred failure publication', async () => {
    ...
    let release!: (value: { outcome: 'preserved'; reason: string; workers: string[] }) => void;
    let resolveCalled!: () => void;
    const called = new Promise<void>(resolve => { resolveCalled = resolve; });
    runtimeMocks.shutdownTeamV2.mockImplementationOnce(async () => {
      resolveCalled();
      return new Promise<{ outcome: 'preserved'; reason: string; workers: string[] }>(resolve => { release = resolve; });
    });
  ...
  it('handleCleanup removes only scoped .omc/state/team/<teamName> directory', async () => {
    ...
    writeFileSync(
      join(jobsDir, `${jobId}.json`),
      JSON.stringify(jobRecord('team-one', cwd)),
      'utf-8',
    );
    writeFileSync(
      join(jobsDir, `${jobId}-panes.json`),
      JSON.stringify(paneArtifact(INSTANCE_ID, ['%2'])),
      'utf-8',
    );
  ...
  it('handleCleanup preserves state when runtime cannot prove pane liveness', async () => {
    ...
    writeFileSync(
      join(jobsDir, `${jobId}-panes.json`),
      JSON.stringify(paneArtifact(INSTANCE_ID, ['%9'])),
      'utf-8',
    );
    runtimeMocks.shutdownTeamV2.mockResolvedValueOnce({
      outcome: 'preserved',
  ...
  it.skipIf(!capturedTmuxServerIdentity)('uses the real instance reservation and cleanup receipt protocol for an empty team', async () => {
    const { handleCleanup } = await importTeamServerWithJobsDir(jobsDir);
    const { createTeamInstanceBinding, reserveTeamInstance, activateTeamInstanceUnderLock, withTeamInstanceLifecycleLock } =
    ...
    runtimeMocks.shutdownTeamV2.mockImplementationOnce(async (team, root, options) =>
    ...
    const receiptPath = absPath(cwd, TeamPaths.teamInstanceCleanupReceipt(
  ...
  it.skipIf(!capturedTmuxServerIdentity)('preserves state when the real cleanup receipt is corrupt', async () => {
    ...
    const cleanupPath = canonicalTeamStatePath(cwd, TeamPaths.teamInstanceCleanupReceipt(
    ...
    const actualShutdown = runtimeMocks.actualShutdownTeamV2;
    expect(actualShutdown).toBeDefined();
    runtimeMocks.shutdownTeamV2.mockImplementation(async (team, root, options) =>
      (await actualShutdown!(
        String(team),
        String(root),
        options as Record<string, unknown>,
