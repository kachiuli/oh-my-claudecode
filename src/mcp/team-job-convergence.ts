...
function readResultArtifact(omcJobsDir: string, jobId: string): ArtifactOutcome {
  ...
  try {
    const parsed = JSON.parse(raw) as { status?: string; instanceId?: unknown };
    if (
      (parsed?.status === 'completed' || parsed?.status === 'failed')
      && typeof parsed.instanceId === 'string'
    ) {
      return { kind: 'terminal', status: parsed.status as 'completed' | 'failed', raw };
    }
...
export function convergeJobWithResultArtifact(
  ...
  if (artifact.kind === 'terminal') {
    let parsed: { instanceId?: unknown };
    try {
      parsed = JSON.parse(artifact.raw) as { instanceId?: unknown };
    } catch {
      return {
        job: {
          ...job,
...
export function resultArtifactIdentityError(
  job: Pick<OmcTeamJob, 'instanceId'>,
  jobId: string,
  omcJobsDir: string,
): string | null {
  const artifactPath = join(omcJobsDir, `${jobId}-result.json`);
  ...
  try {
    const parsed = JSON.parse(raw) as { status?: unknown; instanceId?: unknown };
    if (
      (parsed.status !== 'completed' && parsed.status !== 'failed')
...
export function readMatchingTerminalArtifact(
  job: Pick<OmcTeamJob, 'instanceId'>,
  jobId: string,
  omcJobsDir: string,
): { status: 'completed' | 'failed'; raw: string } | null {
  try {
...
export function isValidOmcTeamJob(value: unknown): value is OmcTeamJob {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return false;
  const record = value as Record<string, unknown>;
  if (
    (record.status !== 'running' && record.status !== 'completed' && record.status !== 'failed' && record.status !== 'timeout')
...
export function isValidTeamPaneArtifact(value: unknown, instanceId: string): value is TeamPaneArtifact {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return false;
  const record = value as Record<string, unknown>;
  if (
